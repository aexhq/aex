...
//!
//! Exclusive responsibility: the credential-free guest-root protocol agent process.
//!
...
/// Validated start-up configuration for `hands-agent`.
///
...
pub struct Config {
    ...
    pub region: String,
    /// Guest-visible control socket path the agent listens on.
    pub resource: String,
...
pub enum RunError {
    ...
    #[error("`hands-agent` has no implementation yet")]
    NotImplemented,
}
...
/// Environment variable naming guest-visible control socket path the agent listens on.
pub const RESOURCE_VAR: &str = "AEX_HANDS_CONTROL_SOCKET";
/// Environment variable naming maximum captured output bytes per command.
pub const BUDGET_VAR: &str = "AEX_MAX_OUTPUT_BYTES";

/// Planes this deployable may be bound to.
const PLANES: [&str; 2] = ["dev", "prd"];

impl Config {
    /// Reads and validates the configuration of `hands-agent` from the process environment.
    ///
    /// # Errors
    ///
...
mod tests {
    ...
    fn complete() -> BTreeMap<&'static str, String> {
        BTreeMap::from([
            ...
            (RESOURCE_VAR, "aex-hands_agent-fixture".to_owned()),
    ...
    fn accepts_a_complete_environment() {
        ...
        assert_eq!(config.resource, "aex-hands_agent-fixture");
