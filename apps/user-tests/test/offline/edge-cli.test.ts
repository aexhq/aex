...
describe("installed aex CLI — offline edge cases", () => {
  ...
  async function runCli(args: readonly string[], timeoutMs = 30_000): Promise<RunResult> {
  ...
  it("no args prints usage and exits 0", async () => {
    ...
    expect(r.stdout).toMatch(/aex run/);
  });
  ...
  it("run rejects an unknown flag with exit 2", async () => {
    const r = await runCli([
      "run",
      "--provider", "anthropic",
      "--anthropic-api-key", "k",
  ...
  it("run with a near-miss model suggests the correct one (exit 2)", async () => {
    const r = await runCli([
      "run",
      "--anthropic-api-key", "k",
      "--model", "claude-haiku",
  ...
  it("run with a near-miss provider suggests the correct one (exit 2)", async () => {
    const r = await runCli([
      "run",
      "--provider", "anthropicc",
      "--anthropic-api-key", "k",
  ...
  it("removed --proxy-endpoint flag on run exits 2 with a migration hint", async () => {
    const r = await runCli([
      "run",
      "--anthropic-api-key", "k",
      "--model", "claude-haiku-4-5",
  ...
  it("providers list --json emits a parseable JSON array", async () => {
    const r = await runCli(["providers", "list", "--json"]);
  ...
  it("a discovery verb with a stray positional arg exits 2", async () => {
    const r = await runCli(["tools", "list", "garbage-arg"]);
