...
describe("pre-create transport failure classifier", () => {
  it("matches Bun-style direct upload ConnectionRefused failures before a session is created", () => {
    const threw =
      "uploadAsset: direct upload PUT failed for https://aex-dev-eu-west-2-files-522921482290.s3.eu-west-2.amazonaws.com/assets/example?[redacted] after 3 attempts (ConnectionRefused): Unable to connect. Is the computer able to access the url?";

    expect(isPreCreateTransportFailure({ sessionId: null, threw })).toBe(true);
  });

  it("matches common nested transport spellings but requires no session id", () => {
    ...
    expect(
      isPreCreateTransportMessage(
        "AexNetworkError: POST dev-api.aex.dev/api/sessions failed: ConnectionRefused (Unable to connect. Is the computer able to access the url?) after 4 attempts"
      )
    ).toBe(true);
    expect(isPreCreateTransportFailure({ sessionId: "ses_0123456789abcdef0123456789abcdef", threw: "fetch failed" })).toBe(false);
  });

  it("does not classify product or assertion failures as pre-create transport gaps", () => {
    expect(isPreCreateTransportFailure({ sessionId: null, threw: "AssertionError: expected assistant text to contain token" })).toBe(false);
    expect(isPreCreateTransportFailure({ sessionId: null, threw: null })).toBe(false);
  });
