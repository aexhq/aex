...
function outputsFetch(call: FetchCall): Response {
  ...
  if (url.pathname === "/api/runs/s1/outputs") {
    return json({ outputs: OUTPUTS });
  }
  if (url.pathname === "/api/runs/s1/outputs/o1/download") {
    return new Response("{\"ok\":1}", {
      status: 200,
      headers: { "content-type": "application/json", "content-length": "8" }
    });
  }
  if (url.pathname === "/api/runs/s1/outputs/o1/link" && call.init.method === "POST") {
...
describe("aex outputs single-file sub-verbs (T6b)", () => {
  it("`outputs read <id> <path>` resolves and reads the output text", async () => {
    ...
    expect(cap.calls.map((call) => new URL(call.url).pathname)).toContain("/api/runs/s1/outputs/o1/download");
  ...
  it("`outputs link <id> <path>` resolves and mints the URL", async () => {
    ...
    await runCli(cap.io);
    expect(cap.exitCode).toBe(0);
    expect(cap.calls.map((call) => new URL(call.url).pathname)).toContain("/api/runs/s1/outputs/o1/link");
  ...
  it("`outputs search --query` searches recent session output metadata", async () => {
    ...
    expect(JSON.parse(cap.stdout.trim()).hits).toEqual([
      expect.objectContaining({ runId: "s1", outputId: "o1", filename: "data.json" })
    ]);
  });
