...
const COMMAND_ERROR_MATRIX = [
  ...
  { source: "billing.ts", path: "billing topup", call: 'emitApiError(io, "billing_topup_failed", err)' },
  { source: "billing.ts", path: "billing autotopup", call: 'emitApiError(io, "billing_autotopup_failed", err)' },
...
describe("SDK/API error source ownership", () => {
  it("covers all 42 current command failure paths through the common emitter", () => {
    expect(COMMAND_ERROR_MATRIX).toHaveLength(42);
