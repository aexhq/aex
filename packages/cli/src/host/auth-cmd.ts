 ...
 * non-interactive fallback: it validates a WORKSPACE (data-plane) key against
 * `whoami` BEFORE writing (a bad token is never persisted) and stores it as
 * `apiKey`. The token value is never printed by any of these verbs.
 *
...
export async function executeLoginCmd(io: CliIO, argv: readonly string[]): Promise<CliExitCode> {
  ...
  const loginUsage = "usage: aex login [--aex-url <url>]  |  aex login --api-key <workspace-key> [--aex-url <url>]";
  ...
  // --api-key → non-interactive fallback: validate a workspace key via whoami.
