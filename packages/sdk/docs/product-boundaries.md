...
| Deployment model | The supported product is the hosted antpath control plane, plus the SDK and CLI used to submit and inspect runs. | True self-host and customer-cloud deployments are not supported product modes today. Alternate `baseUrl` values are for local, staging, or hosted antpath API planes, not a self-host promise. |
