...
| Deployment model | The repository contains source for the SDK, CLI, contracts, conformance helpers, user-test harness, and docs. | Hosted platform implementation, deployment workflows, billing/rate logic, and provider/substrate adapters are private. Public source visibility is not a supported deployment boundary: true self-host and customer-cloud deployments are not supported product modes today. Alternate `baseUrl` values are for local, staging, private, or hosted antpath API planes, not a self-host promise. |
