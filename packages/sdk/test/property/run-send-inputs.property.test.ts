...
import { Aex, type Message, type RunResult, type SessionInput, type SessionRunOptions } from "../../src/index.js";
...
function runOptions(input: SessionInput, keys: IdempotencyCase = {}): SessionRunOptions {
  return {
    model: MODEL,
    message: input,
    apiKeys: API_KEYS,
...
function expectRunKeys(harness: Harness, keys: IdempotencyCase): void {
...
function assertNoRunEndpoint(harness: Harness): void {
  expect(harness.calls.some((call) => call.url.includes("/api/runs"))).toBe(false);
}
...
describe("SDK run/send SessionInput properties", () => {
  it("Aex.run serializes valid message inputs and uses predictable idempotency keys", async () => {
    await fc.assert(
      fc.asyncProperty(validSessionInput, idempotencyCase, async (input, keys) => {
        const harness = makeHarness();
        const promise = harness.client.run(runOptions(input, keys), { webSocketFactory: harness.webSocketFactory });
        await finishTurn(harness);
        const result = await promise;

        expect(messageCalls(harness)[0]!.body).toEqual({ input });
        expectRunKeys(harness, keys);
        expect(result.text).toBe("");
        assertNoRunEndpoint(harness);
      }),
      PROPERTY_RUNS
    );
  });

  it("sessions.run serializes valid message inputs and uses predictable idempotency keys", async () => {
    await fc.assert(
      fc.asyncProperty(validSessionInput, idempotencyCase, async (input, keys) => {
        const harness = makeHarness();
        const promise = harness.client.sessions.run({
  ...
  it("rejects invalid Aex.run message inputs before any HTTP request", async () => {
    await fc.assert(
      fc.asyncProperty(invalidSessionInput, async (input) => {
        const harness = makeHarness();

        await expect(
          harness.client.run({ ...runOptions("fallback"), message: input } as unknown as SessionRunOptions)
  ...
  it("rejects invalid sessions.run message inputs before any HTTP request", async () => {
    await fc.assert(
      fc.asyncProperty(invalidSessionInput, async (input) => {
        const harness = makeHarness();

        await expect(
          harness.client.sessions.run({ ...runOptions("fallback"), message: input } as unknown as SessionRunOptions)
        ).rejects.toBeInstanceOf(Error);
        expect(harness.calls).toHaveLength(0);
      }),
      PROPERTY_RUNS
  ...
  it("Aex.run is create plus send and projects fuzzed TEXT_MESSAGE_CONTENT streams consistently", async () => {
    await fc.assert(
      fc.asyncProperty(validSessionInput, textEventSequence, async (input, specs) => {
        ...
        expect(messageCalls(harness)[0]!.body).toEqual({ input });
        expectRunKeys(harness, { createKey: "create_key" });
        assertRunEventProjection(result, specs);
        assertNoRunEndpoint(harness);
      }),
      EVENT_PROPERTY_RUNS
    );
