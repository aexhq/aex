...
describe("SDK model did-you-mean (WS5)", () => {
  it("a typo'd known model throws with a 'did you mean' hint before any network", async () => {
    ...
    const err = await client
      .run({ model: "deepseek-v4-flsh", message: "hi", apiKeys: { deepseek: "K" } } as never)
      .catch((e: unknown) => e);
    expect(err).toBeInstanceOf(RunConfigValidationError);
    expect((err as Error).message).toMatch(/did you mean "deepseek-v4-flash"/);
    expect((err as RunConfigValidationError).details).toMatchObject({ field: "model" });
  });
});
