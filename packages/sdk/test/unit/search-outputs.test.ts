/**
 * `AgentExecutor.searchOutputs` (chat-mvp) — client-side cross-run output search.
 * Drives a real AgentExecutor with a fake fetch so the contracts output filter
 * (filename / extension / contentType) runs end-to-end.
 */
...
function makeClient(): { client: AgentExecutor; calls: string[] } {
  ...
  const fetchImpl: typeof fetch = vi.fn(async (input) => {
    ...
    calls.push(url);
    const m = /\/api\/runs\/([^/]+)\/outputs$/.exec(url);
    if (m) return jsonResponse({ outputs: OUTPUTS[m[1]!] ?? [] });
    if (/\/api\/runs(\?|$)/.test(url)) {
      return jsonResponse({ runs: [{ id: "run-a" }, { id: "run-b" }] });
    }
...
describe("AgentExecutor.searchOutputs", () => {
  it("filters by filename substring across the corpus and returns references only", async () => {
    const { client } = makeClient();
    const page = await client.searchOutputs({ runIds: ["run-a", "run-b"], filename: "report" });
  ...
  it("filters by extension", async () => {
    const { client } = makeClient();
    const page = await client.searchOutputs({ runIds: ["run-a", "run-b"], extension: "json" });
    expect(page.hits.map((h) => h.outputId)).toEqual(["a2"]);
  });

  it("filters by content type wildcard", async () => {
    const { client } = makeClient();
    const page = await client.searchOutputs({ runIds: ["run-a", "run-b"], contentType: "image/*" });
    expect(page.hits.map((h) => h.outputId)).toEqual(["b2"]);
  });

  it("honors the limit", async () => {
    const { client } = makeClient();
    const page = await client.searchOutputs({ runIds: ["run-a", "run-b"], limit: 1 });
    expect(page.hits).toHaveLength(1);
    expect(page.hits[0]!.runId).toBe("run-a");
  });

  it("falls back to the whole workspace via listRuns when no runIds given", async () => {
    const { client, calls } = makeClient();
    const page = await client.searchOutputs({ extension: "md" });
    expect(page.hits.map((h) => h.outputId)).toEqual(["a1", "b1"]);
    expect(calls.some((u) => /\/api\/runs(\?|$)/.test(u))).toBe(true);
  });
});
