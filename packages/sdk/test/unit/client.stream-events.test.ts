/**
 * SDK-level coverage for `AntpathClient.streamEvents` (the loose `RunEvent`
...
describe("AntpathClient.streamEvents — polling the coordinator-backed /events", () => {
  it("yields events, dedupes by id across polls, and stops on terminal status", async () => {
    ...
    const client = new AntpathClient({ apiToken: "tk", baseUrl: "https://dash.test", fetch: f });
    const events: string[] = [];
