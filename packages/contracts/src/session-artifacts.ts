 ...
 *
 * Every session stores public outputs and internal diagnostics under separate
 * prefixes:
 *
 *   sessions/<sessionId>/outputs/<rel>              — the session's real deliverables.
 *   sessions/<sessionId>/internal/logs/<rel>        — platform diagnostics
 *                                             (`runtime/`, `host/`,
 *                                             `provider-proxy/`,
 *                                             `control-plane/`).
 *
...
export const SESSION_OUTPUTS_PREFIX = "outputs";
export const SESSION_INTERNAL_LOGS_PREFIX = "internal/logs";

/**
 * Relative-path prefixes that mark a stored artifact as a platform diagnostic
 * rather than a session deliverable. Dotted forms are upload-time paths; legacy
...
export function isSessionLogRelPath(rel: string): boolean {
  return SESSION_LOG_REL_PREFIXES.some((prefix) => rel.startsWith(prefix));
}
 ...
 */
export function sessionArtifactKey(sessionId: string, rel: string): string {
  const namespace = isSessionLogRelPath(rel) ? SESSION_INTERNAL_LOGS_PREFIX : SESSION_OUTPUTS_PREFIX;
  return `sessions/${sessionId}/${namespace}/${sessionArtifactRel(rel)}`;
}

/** The `assets` namespace under a session's prefix. */
export const SESSION_ASSETS_PREFIX = "assets";

/**
 * Storage key for a session's snapshotted asset, addressed by its content hash.
 * At submit, each referenced shared-store asset is copied here so the session owns
 * its bytes. Accepts a `sha256:<hex>` hash or a bare 64-hex digest; the stored
 * suffix is the hex.
 */
export function sessionAssetKey(sessionId: string, hash: string): string {
  const hex = hash.startsWith("sha256:") ? hash.slice("sha256:".length) : hash;
  return `sessions/${sessionId}/${SESSION_ASSETS_PREFIX}/${hex}`;
}
