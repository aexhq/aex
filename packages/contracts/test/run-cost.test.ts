...
import {
  RUN_COST_TELEMETRY_SCHEMA_VERSION,
  RUN_USAGE_SAMPLE_SCHEMA_VERSION,
  buildRunCostTelemetry,
  buildRunCostTelemetryFromUsageSamples,
  buildRunUsageSample,
  mergeRunCostTelemetry
} from "../src/index.js";

describe("run cost telemetry", () => {
  ...
  it("records duration, output, retry, capture, and provider usage data", () => {
    ...
    expect(telemetry.schemaVersion).toBe(RUN_COST_TELEMETRY_SCHEMA_VERSION);
  ...
  it("requires source-linked usage samples with supported metric units", () => {
    const sample = buildRunUsageSample({
    ...
    expect(sample.schemaVersion).toBe(RUN_USAGE_SAMPLE_SCHEMA_VERSION);
    ...
    const telemetry = buildRunCostTelemetryFromUsageSamples({
      ...
      samples: [
        ...
        },
        {
          ...
          quantity: 2,
          source: { type: "run-event", id: "evt_retry_summary" }
        },
        {
  ...
  it("merges additive samples without mutating the existing telemetry", () => {
    const first = buildRunCostTelemetry({
      provider: "anthropic",
    ...
    const merged = mergeRunCostTelemetry(first, {
  ...
  it("rejects negative or non-finite numbers", () => {
    expect(() => buildRunCostTelemetry({ durations: { runtimeMs: -1 } })).toThrow(
      /runtimeMs must be a non-negative finite number/
    );
    expect(() =>
      buildRunCostTelemetry({ providerUsage: [{ provider: "anthropic", totalTokens: Number.NaN }] })
