...
describe("session custody manifest contract", () => {
      ...
      resources: [
        {
          ...
          count: 1,
          exposures: [{ surface: "session_artifact_store", access: "stored", status: "revoked" }],
          disposition: {
            status: "destroyed",
        ...
        },
        {
          ...
          disposition: {
            ...
            reason: "ses_retained_until_user_delete",
            decidedAt: "2026-06-02T10:05:02.000Z"
          }
        }
      ],
  ...
  it("writes through the public writer interface without embedding object keys in the manifest", async () => {
    ...
    expect(result).toMatchObject({
      ...
      sessionId: "session-11111111",
      workspaceId: "workspace-11111111",
      key: custodyManifestObjectKey("session-11111111")
    });
    expect(store.listKeys()).toEqual([custodyManifestObjectKey("session-11111111")]);

    const stored = store.getBySessionId("session-11111111");
    expect(stored?.summary.secretInstanceCount).toBe(0);
    expect(JSON.stringify(stored)).not.toContain("sessions/session-11111111/");
  ...
  it("does NOT flag content-addressed hashes, URLs, /workspace paths, or tool-result text (over-masking fix)", () => {
    ...
    // Skill-pack documentation prose that chains the handle-keyword set
    // (agent/file/skill/resource/session/…) with `_`/`-` — these are ordinary
    // compound words in markdown docs read by `read_file`, NOT private handles.
    // (The reported over-masking blocked download() on exactly this text.)
    const docProse =
      "agent_decision_failure is logged when agent-judgment is uncertain; an " +
      "agent-callable surfaces resource_retrieval and file_grounded steps, and " +
      "session_handoff_contract / skill_invocation document env_variable_name usage.";
    ...
    const survivors: ReadonlyArray<readonly [string, unknown]> = [
      ...
      ["handle-keyword doc prose", docProse],
      ["agent_decision_failure token", "agent_decision_failure"],
      ["agent-judgment token", "agent-judgment"],
      ["agent-callable token", "agent-callable"],
      ["resource_retrieval token", "resource_retrieval"],
      ["file_grounded token", "file_grounded"],
      ["session_handoff_contract token", "session_handoff_contract"],
      ["read_file tool_result of skill doc", { data: { content: [{ type: "text", text: docProse }] } }]
    ];
  ...
  it("STILL flags real provider keys, bearers, and opaque secret blobs (no weakening)", () => {
    ...
    // The catch-all still fires when a content-hash-LENGTH-mismatched opaque sequence
    ...
    for (const h of [
      ...
      expect(scanCustodyPayloadForSensitiveValues(h).map((f) => f.reason), h).toContain(
