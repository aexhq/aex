...
import { CustomerEnvironment, customerTerminalDigest } from "../dist/index.js";
...
function callback(input, handler, { registration, output } = {}) {
  const name = handler.name;
  const contractDigest = createHash("sha256").update(JSON.stringify({
    name,
    input: z.toJSONSchema(input),
    ...(output === undefined ? {} : { output: z.toJSONSchema(output) }),
  })).digest("hex");
  return { registration: registration ?? `tool:${contractDigest}`, name, contractDigest, input, output, handler };
}

async function readyEnvironment(registration) {
  ...
  const environment = new CustomerEnvironment(
    async () => ({
      request: { url: "wss://environment.invalid", protocol: "environment-grant.short-lived" },
  ...
  await turn();
  socket.open();
  const hello = JSON.parse(socket.sent.shift());
  assert.equal(hello.type, "register");
  assert.equal(hello.client_id, "app.primary");
  assert.equal(hello.process_id, "process.one");
  assert.deepEqual(request, { url: "wss://environment.invalid", protocol: "environment-grant.short-lived" });
  ...
  return { socket, environment, registration, observations };
}

test("customer Environment receipts, validates, executes, and retains one exact operation", async () => {
  let executions = 0;
  const echo = callback(z.object({ text: z.string() }), async function echo(input) {
    executions += 1;
    return input;
  }, { output: z.object({ text: z.string() }), registration: "echo.current" });
  const { socket, environment, registration, observations } = await readyEnvironment(echo);
  const offer = {
    type: "offer",
    epoch: 7,
    operation_id: "op-1",
...
test("customer Environment cancellation reaches the exact running operation", async () => {
  const wait = callback(z.object({}), async function wait(_input, context) {
    return await new Promise((_resolve, reject) => {
      context.signal.addEventListener("abort", () => reject(new Error("observed abort")), { once: true });
    });
  }, { registration: "wait.current" });
  const { socket, environment, registration, observations } = await readyEnvironment(wait);
  socket.message({
    type: "offer",
    epoch: 7,
...
test("routine socket loss reconnects without cancelling or redispatching an assigned operation", async () => {
  let executions = 0;
  let finish;
  const registration = callback(z.object({}), async function work(_input, context) {
    executions += 1;
    return await new Promise((resolve, reject) => {
      finish = resolve;
      context.signal.addEventListener("abort", () => reject(new Error("aborted")), { once: true });
    });
  }, { registration: "work.current" });
  const sockets = [];
  const observations = [];
  let grants = 0;
  const environment = new CustomerEnvironment(
...
test("retention capacity applies backpressure and never evicts an unacknowledged terminal", async () => {
  let executions = 0;
  const registration = callback(z.object({ value: z.number() }), async function bounded(input) {
    executions += 1;
    return input;
  }, { registration: "bounded.current" });
  const socket = new FakeSocket();
  const observations = [];
  const environment = new CustomerEnvironment(
    async () => ({
...
test("inline result bound counts UTF-8 bytes", async () => {
  const value = callback(z.object({}), async function unicode() {
    return "界".repeat(45_000);
  }, { registration: "unicode.current" });
  const { socket, environment, registration, observations } = await readyEnvironment(value);
  socket.message({
...
test("a running operation rejects a different digest without aliasing its receipt", async () => {
  let executions = 0;
  let finish;
  const value = callback(z.object({}), async function blocked() {
    executions += 1;
    return await new Promise((resolve) => { finish = resolve; });
  }, { registration: "blocked.digest" });
  const { socket, environment, registration, observations } = await readyEnvironment(value);
  const offer = {
...
test("a failed receipt delivery removes pre-effect admission and retries safely", async () => {
  ...
  const registration = callback(z.object({}), async function once() {
    executions += 1;
    return { executions };
  }, { registration: "receipt.retry" });
  const socket = new FakeSocket();
  const observations = [];
  const environment = new CustomerEnvironment(
    async () => ({
...
test("an ambiguous success delivery retains one immutable terminal outcome", async () => {
  ...
  const registration = callback(z.object({}), async function mutableResult() {
    executions += 1;
    return {
      toJSON() {
        serializations += 1;
  ...
  }, { registration: "terminal.immutable" });
  const socket = new FakeSocket();
  const observations = [];
  const environment = new CustomerEnvironment(
    async () => ({
...
test("same registration and contract never aliases a different closure", async () => {
  const firstRegistration = callback(
    z.object({}),
    async function same() { return { source: "first" }; },
    { registration: "same.contract" },
  );
  const secondRegistration = callback(
    z.object({}),
    async function same() { return { source: "second" }; },
    { registration: "same.contract" },
  );
