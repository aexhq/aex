...
import type {
  FileEntry as CanonicalEnvironmentFileEntry,
  SandboxStatus as CanonicalEnvironmentStatus,
...
export type EnvironmentStatus = CanonicalEnvironmentStatus;

/** Exact canonical Environment file metadata; timestamps are integer Unix milliseconds. */
export type EnvironmentFileEntry = CanonicalEnvironmentFileEntry;

export type EnvironmentFileList = ApiSchemas["SandboxFileList"];
...
export class EnvironmentFiles {
  ...
  private path(suffix: string): string {
    return `/v1/sessions/${sid(this.sessionId)}/environments/${encodeURIComponent(this.environment)}/files/${suffix}`;
  }

  list(input: ApiSchemas["SandboxFileListRequest"], options: JsonRequestOptions = {}): Promise<EnvironmentFileList> {
    return this.transport.json("POST", this.path("list"), { ...options, body: input });
  }

  stat(input: ApiSchemas["SandboxFilePathRequest"], options: JsonRequestOptions = {}): Promise<EnvironmentFileEntry> {
    return this.transport.json("POST", this.path("stat"), { ...options, body: input });
  }

  readInline(input: ApiSchemas["SandboxFilePathRequest"], options: JsonRequestOptions = {}): Promise<ApiSchemas["SandboxFileContent"]> {
    return this.transport.json("POST", this.path("read-inline"), { ...options, body: { ...input, max_bytes: MAX_INLINE_FILE_BYTES } });
  }

  writeInline(input: ApiSchemas["SandboxFileWriteRequest"], options: JsonRequestOptions = {}): Promise<EnvironmentFileEntry> {
    assertInlineBase64(input.content_base64);
    return this.transport.json("POST", this.path("write-inline"), { ...idempotent(options), body: input });
  }
  ...
  prepareDownload(input: ApiSchemas["SandboxFilePathRequest"], options: JsonRequestOptions = {}): Promise<TransferTicket> {
    return this.transport.json("POST", this.path("downloads"), { ...options, body: input });
  }
  ...
  prepareUpload(input: ApiSchemas["SandboxFileUploadRequest"], options: JsonRequestOptions = {}): Promise<TransferTicket> {
    return this.transport.json("POST", this.path("uploads"), { ...options, body: input });
  }

  /** Not auto-retried. Use storage plus copy when the transfer itself must be recovery-safe. */
  completeUpload(transferId: string, options: JsonRequestOptions = {}): Promise<EnvironmentFileEntry> {
    return this.transport.json("POST", this.path(`uploads/${encodeURIComponent(transferId)}/complete`), options);
  }

  find(input: ApiSchemas["SandboxFileFindRequest"], options: JsonRequestOptions = {}): Promise<EnvironmentFileList> {
    return this.transport.json("POST", this.path("find"), { ...options, body: input });
  }

  grep(input: ApiSchemas["SandboxFileGrepRequest"], options: JsonRequestOptions = {}): Promise<EnvironmentFileList> {
    return this.transport.json("POST", this.path("grep"), { ...options, body: input });
  }
}
...
export class SessionEnvironment {
  ...
  constructor(
    ...
    this.files = new EnvironmentFiles(transport, sessionId, environment);
  }

  status(options: JsonRequestOptions = {}): Promise<EnvironmentStatus> {
    return this.transport.json("GET", `/v1/sessions/${sid(this.sessionId)}/environments/${encodeURIComponent(this.environment)}`, options);
  }

  create(options: JsonRequestOptions = {}): Promise<EnvironmentStatus> {
    return this.transport.json("POST", `/v1/sessions/${sid(this.sessionId)}/environments/${encodeURIComponent(this.environment)}`, options);
  }
}
...
export class SessionStorage {
  ...
  copyToEnvironment(environment: string, input: ApiSchemas["StorageEnvironmentCopyRequest"], options: JsonRequestOptions = {}): Promise<EnvironmentFileEntry> {
