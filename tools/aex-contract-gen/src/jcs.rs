...
fn normalize(value: &mut Value) {
    match value {
        ...
        Value::Object(members) => {
            ...
            // See `aex_wire::canonical::normalize`: `serde_json::Map` is only a
            // `BTreeMap` while `preserve_order` is off, and an AWS SDK crate in
            // the build turns it on. Sort explicitly so the generated bytes do
            // not depend on which crates share a compilation.
