...
const workspaces = ["contracts", "environment", "session-protocol", "sdk", "cli"];
...
const pack = async (directory) => {
  ...
  const manifest = { schema: 1, source: process.env.GITHUB_SHA ?? "local", packages };
  await writeFile(path.join(directory, "manifest.json"), `${JSON.stringify(manifest, null, 2)}\n`);
  // Publication consumes these reviewed copies from the immutable workflow artifact. It never
  // downloads an operator script from a mutable branch between build and registry mutation.
