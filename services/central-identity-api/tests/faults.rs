//! `central-identity-api` fail-closed configuration evidence.

use std::process::Command;

#[test]
fn the_artifact_refuses_to_start_without_its_exact_configuration() {
    let output = Command::new(env!("CARGO_BIN_EXE_central-identity-api"))
        .env_clear()
        .output()
        .expect("`central-identity-api` starts");
