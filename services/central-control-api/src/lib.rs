//! `central-control-api`: the control half of the central HTTP surface.
//!
...
//!
//! `services/central-api` composes these five route groups, the two device-flow
//! routes and the billing group into **one** long-lived Fargate process. A
//! deployable whose service type lives in a module private to its own `main.rs`
//! cannot be composed into another binary at all, so the handlers, the
...
//! Nothing about the Lambda deployable changed: [`run`] still serves through
//! `lambda_http`, and the merged service reaches the same [`api::ControlService`]
//! through the same [`ControlApi`] bound.
...
pub const DEPLOYABLE: CentralServiceId = CentralServiceId::ControlApi;
