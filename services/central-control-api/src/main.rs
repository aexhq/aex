...
//!
//! Exclusive responsibility: organizations, members, workspaces, keys, durable operations
//! and audit.
//!
...
/// Validated start-up configuration for `central-control-api`.
///
...
pub struct Config {
    ...
    pub region: String,
    /// Aurora cluster holding the control schema.
    pub resource: String,
    /// Maximum page size accepted by a list route.
    pub budget: u32,
}
...
pub enum RunError {
    ...
    #[error("`central-control-api` has no implementation yet")]
    NotImplemented,
}
...
/// Environment variable naming Aurora cluster holding the control schema.
pub const RESOURCE_VAR: &str = "AEX_CONTROL_DB_CLUSTER_ARN";
/// Environment variable naming maximum page size accepted by a list route.
pub const BUDGET_VAR: &str = "AEX_MAX_PAGE_SIZE";

/// Planes this deployable may be bound to.
const PLANES: [&str; 2] = ["dev", "prd"];

impl Config {
    /// Reads and validates the configuration of `central-control-api` from the process environment.
    ///
    /// # Errors
    ///
...
fn main() -> std::process::ExitCode {
    ...
    {
        eprintln!("central-control-api: telemetry flush left {pending} record(s) undelivered");
    }
    match outcome {
...
mod tests {
    ...
    fn complete() -> BTreeMap<&'static str, String> {
        BTreeMap::from([
            ...
            (RESOURCE_VAR, "aex-central_control_api-fixture".to_owned()),
    ...
    fn accepts_a_complete_environment() {
        ...
        assert_eq!(config.resource, "aex-central_control_api-fixture");
