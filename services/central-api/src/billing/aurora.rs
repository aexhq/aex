...
use aex_payment_contracts::{
    CommandKind, EffectId, PaymentResult, ProviderCustomerRef, ProviderMethodRef, RedactedEmail,
};
use aex_rds_data::{
...
use super::authority::{
    AuthorityError, BalanceRecord, BillingAuthority, EffectPreparation, PaymentMethodRecord,
    PolicyChange, PolicyRecord, StatementHeader, StatementLineRecord, StatementPage,
    TransactionPageRecord, TransactionRecord, UsagePageRecord, UsageQuery, UsageRecord,
};
...
SELECT GREATEST(coalesce((SELECT -sum(b.balance_microusd) FROM finance.account_balance b \
 ...
 WHERE ba.org_id = :org_id";
    ...
    /// Verified, display-only cards attached to one account.
    pub const LIST_PAYMENT_METHODS: &str = "\
SELECT pm.payment_method_id, pm.provider_method_id, pm.brand, pm.last4, \
       pm.expiry_month, pm.expiry_year, \
       (ba.default_payment_method_id = pm.provider_method_id), \
       (EXTRACT(EPOCH FROM pm.created_at)*1000)::bigint \
  FROM finance.payment_method pm \
  JOIN finance.billing_account ba ON ba.org_id = pm.org_id \
 WHERE pm.org_id = :org_id AND pm.state = 'attached' \
 ORDER BY pm.created_at DESC, pm.payment_method_id DESC \
 LIMIT 100";

    /// One account-owned card, including its provider reference for detach.
    pub const READ_PAYMENT_METHOD: &str = "\
SELECT pm.payment_method_id, pm.provider_method_id, pm.brand, pm.last4, \
       pm.expiry_month, pm.expiry_year, \
       (ba.default_payment_method_id = pm.provider_method_id), \
       (EXTRACT(EPOCH FROM pm.created_at)*1000)::bigint \
  FROM finance.payment_method pm \
  JOIN finance.billing_account ba ON ba.org_id = pm.org_id \
 WHERE pm.org_id = :org_id AND pm.payment_method_id = :payment_method_id \
   AND pm.state = 'attached'";

    /// Customer-visible immutable transactions with a stable same-account cursor.
    pub const LIST_TRANSACTIONS: &str = "\
WITH boundary AS ( \
  SELECT occurred_at, transaction_id FROM finance.journal_transaction \
   WHERE org_id = :org_id AND transaction_id = :after_id \
), visible AS ( \
  SELECT jt.transaction_id, jt.kind::text AS kind, jt.reverses_transaction_id, \
         jt.occurred_at, \
         coalesce(-sum(jp.amount_microusd) FILTER (WHERE a.kind = 'customer_available'), 0)::bigint \
           AS available_delta, \
         (SELECT coalesce(-sum(jp2.amount_microusd), 0)::bigint \
            FROM finance.journal_transaction jt2 \
            JOIN finance.journal_posting jp2 ON jp2.transaction_id = jt2.transaction_id \
            JOIN finance.account a2 ON a2.account_id = jp2.account_id \
           WHERE jt2.org_id = jt.org_id AND a2.kind = 'customer_available' \
             AND (jt2.occurred_at, jt2.transaction_id) <= (jt.occurred_at, jt.transaction_id)) \
           AS balance_after, \
         (SELECT r.scope_id FROM finance.usage_inbox ui \
            JOIN finance.reservation r ON r.reservation_id = ui.reservation_id \
           WHERE ui.transaction_id = jt.transaction_id AND r.scope_kind = 'session' LIMIT 1) \
           AS session_id \
    FROM finance.journal_transaction jt \
    JOIN finance.journal_posting jp ON jp.transaction_id = jt.transaction_id \
    JOIN finance.account a ON a.account_id = jp.account_id \
   WHERE jt.org_id = :org_id \
     AND jt.kind IN ('top_up_settled','usage_settlement','refund','reversal', \
                     'usage_reserve','reservation_release') \
     AND (:after_id = '00000000-0000-0000-0000-000000000000'::uuid \
          OR (jt.occurred_at, jt.transaction_id) < \
             (SELECT occurred_at, transaction_id FROM boundary)) \
   GROUP BY jt.transaction_id, jt.kind, jt.reverses_transaction_id, jt.occurred_at, jt.org_id \
) \
SELECT transaction_id, kind, available_delta, balance_after, reverses_transaction_id, \
       session_id, (EXTRACT(EPOCH FROM occurred_at)*1000)::bigint \
  FROM visible ORDER BY occurred_at DESC, transaction_id DESC LIMIT :page_limit";

    /// Rated usage and an explicit account-wide settlement coverage boundary.
    pub const LIST_USAGE: &str = "\
WITH coverage AS ( \
  SELECT coalesce((EXTRACT(EPOCH FROM min(interval_start) FILTER \
           (WHERE state IN ('pending','quarantined')))*1000)::bigint, \
           (EXTRACT(EPOCH FROM max(interval_end))*1000)::bigint, 0) AS complete_through, \
         coalesce((EXTRACT(EPOCH FROM max(interval_end))*1000)::bigint, 0) AS includes_through, \
         bool_or(state = 'quarantined') AS has_gap \
    FROM finance.usage_inbox WHERE org_id = :org_id \
) \
SELECT ui.accepted_sequence, ui.category, ui.meter, ui.quantity::text, \
       coalesce(ui.charged_quantity, ui.quantity)::text, coalesce(ui.rated_microusd, 0)::bigint, \
       (ui.category = 'model' OR ui.transaction_id IS NOT NULL), ui.provider, ui.model, \
       coalesce(ui.session_id, CASE WHEN r.scope_kind = 'session' THEN r.scope_id ELSE NULL END), \
       (EXTRACT(EPOCH FROM ui.interval_start)*1000)::bigint, \
       (EXTRACT(EPOCH FROM ui.interval_end)*1000)::bigint, \
       coverage.complete_through, coverage.includes_through, coalesce(coverage.has_gap, false) \
  FROM finance.usage_inbox ui \
  LEFT JOIN finance.reservation r ON r.reservation_id = ui.reservation_id \
 CROSS JOIN coverage \
 WHERE ui.org_id = :org_id AND ui.state = 'rated' \
   AND (:after_sequence < 0 OR ui.accepted_sequence > :after_sequence) \
   AND (:category = '' OR ui.category = :category) \
   AND (:session_id = '' OR ui.session_id = :session_id \
        OR (r.scope_kind = 'session' AND r.scope_id = :session_id)) \
   AND (:from_ms < 0 OR ui.interval_end >= \
        TIMESTAMPTZ 'epoch' + :from_ms * INTERVAL '1 millisecond') \
   AND (:to_ms < 0 OR ui.interval_start < \
        TIMESTAMPTZ 'epoch' + :to_ms * INTERVAL '1 millisecond') \
 ORDER BY ui.accepted_sequence ASC, ui.region, ui.category, ui.fact_id \
 LIMIT :page_limit";
...
pub const fn durable_effect_kind(kind: CommandKind) -> Option<&'static str> {
    match kind {
        ...
        CommandKind::CreatePaymentMethodSession => Some("payment_method_session_create"),
        CommandKind::DetachPaymentMethod => Some("payment_method_detach"),
        CommandKind::RefundCharge => Some("refund_create"),
        CommandKind::LookupEffectOutcome => None,
    }
}
...
impl Row for PaymentMethodRow {
    fn from_record(record: &Record<'_>) -> Result<Self, DecodeError> {
        record.expect_arity(8)?;
        Ok(Self {
            id: record.uuid(0)?,
            provider_method: record.text(1)?.to_owned(),
            brand: record.text(2)?.to_owned(),
            last4: record.text(3)?.to_owned(),
            expiry_month: record.i64(4)?,
            expiry_year: record.i64(5)?,
            is_default: record.bool(6)?,
            created_at_millis: record.i64(7)?,
        })
    }
}

/// One customer-visible transaction row.
#[derive(Debug)]
...
impl Row for TransactionRow {
    fn from_record(record: &Record<'_>) -> Result<Self, DecodeError> {
        ...
        Ok(Self {
            ...
            available_delta: record.i64(2)?,
            balance_after: record.i64(3)?,
            reverses: record.opt(4, Record::uuid)?,
            session_id: record.opt(5, |row, index| row.text(index).map(str::to_owned))?,
            occurred_at_millis: record.i64(6)?,
        })
    }
}

/// One rated usage row with repeated page coverage evidence.
#[derive(Debug)]
struct UsageRow {
    ...
    from_millis: i64,
    to_millis: i64,
    complete_through_millis: i64,
    includes_through_millis: i64,
    has_gap: bool,
}

impl Row for UsageRow {
    fn from_record(record: &Record<'_>) -> Result<Self, DecodeError> {
        record.expect_arity(15)?;
        Ok(Self {
            accepted_sequence: record.i64(0)?,
            category: record.text(1)?.to_owned(),
            meter: record.text(2)?.to_owned(),
            quantity: record
                .text(3)?
                .parse()
                .map_err(|_| DecodeError::Overflow { index: 3 })?,
            charged_quantity: record
                .text(4)?
                .parse()
                .map_err(|_| DecodeError::Overflow { index: 4 })?,
            rated: record.i64(5)?,
            settled: record.bool(6)?,
            provider: record.opt(7, |row, index| row.text(index).map(str::to_owned))?,
            model: record.opt(8, |row, index| row.text(index).map(str::to_owned))?,
            session_id: record.opt(9, |row, index| row.text(index).map(str::to_owned))?,
            from_millis: record.i64(10)?,
            to_millis: record.i64(11)?,
            complete_through_millis: record.i64(12)?,
            includes_through_millis: record.i64(13)?,
            has_gap: record.bool(14)?,
        })
    }
}
...
impl BillingAuthority for AuroraBillingAuthority {
    ...
    async fn payment_methods(
        ...
        let rows: Vec<PaymentMethodRow> = self
            .client
            .query(Statement::with(
                sql::LIST_PAYMENT_METHODS,
                vec![("org_id", SqlValue::Uuid(org_uuid(organization)))],
            ))
            .await
            .map_err(store)?;
        rows.into_iter().map(payment_method_record).collect()
    }
    ...
    async fn payment_method(
        ...
        let row: PaymentMethodRow = self
            .client
            .query_opt(Statement::with(
                sql::READ_PAYMENT_METHOD,
                vec![
                    ("org_id", SqlValue::Uuid(org_uuid(organization))),
                    ("payment_method_id", SqlValue::Uuid(payment_method)),
                ],
            ))
            .await
            .map_err(store)?
            .ok_or(AuthorityError::NotFound("payment method"))?;
        payment_method_record(row)
    }
    ...
    async fn transactions(
        ...
        let requested = usize::try_from(limit).unwrap_or(100);
        let rows: Vec<TransactionRow> = self
            .client
            .query(Statement::with(
                sql::LIST_TRANSACTIONS,
                vec![
                    ("org_id", SqlValue::Uuid(org_uuid(organization))),
                    (
                        "after_id",
                        SqlValue::Uuid(after.unwrap_or(uuid::Uuid::nil())),
                    ),
                    (
                        "page_limit",
                        SqlValue::I64(i64::from(limit.saturating_add(1))),
                    ),
                ],
            ))
            .await
        ...
        let next = has_more.then(|| items.last().expect("a non-empty page").id);
        Ok(TransactionPageRecord { items, next })
    }
    ...
    async fn usage(
        ...
        let requested = usize::try_from(query.limit).unwrap_or(100);
        let rows: Vec<UsageRow> = self
            ...
            .query(Statement::with(
                ...
                vec![
                    ...
                    (
                        "after_sequence",
                        SqlValue::I64(query.after_sequence.unwrap_or(-1)),
                    ),
                    (
                        "category",
                        SqlValue::Text(query.category.unwrap_or_default().to_owned()),
                    ),
                    (
                        "session_id",
                        SqlValue::Text(query.session_id.unwrap_or_default().to_owned()),
                    ),
                    ("from_ms", SqlValue::I64(query.from_millis.unwrap_or(-1))),
                    ("to_ms", SqlValue::I64(query.to_millis.unwrap_or(-1))),
                    (
                        "page_limit",
                        SqlValue::I64(i64::from(query.limit.saturating_add(1))),
                    ),
                ],
            ))
            .await
            .map_err(store)?;
        let has_more = rows.len() > requested;
        let coverage = rows.first().map_or((0, 0, false), |row| {
            (
                row.complete_through_millis,
                row.includes_through_millis,
                row.has_gap,
            )
        });
        let mut items = Vec::with_capacity(rows.len().min(requested));
        for row in rows.into_iter().take(requested) {
            items.push(UsageRecord {
                ...
                rated: money(row.rated, "rated_microusd")?,
                settled: row.settled,
                provider: row.provider,
                model: row.model,
                session_id: row.session_id,
                from_millis: row.from_millis,
                to_millis: row.to_millis,
            });
        }
        let next_sequence =
            has_more.then(|| items.last().expect("a non-empty page").accepted_sequence);
        Ok(UsagePageRecord {
            items,
            next_sequence,
            complete_through_millis: coverage.0,
            includes_through_millis: coverage.1,
            has_gap: coverage.2,
        })
    }
...
fn payment_method_record(row: PaymentMethodRow) -> Result<PaymentMethodRecord, AuthorityError> {
    Ok(PaymentMethodRecord {
        id: row.id,
        provider_method: ProviderMethodRef(row.provider_method),
        brand: row.brand,
        last4: row.last4,
        expiry_month: u32::try_from(row.expiry_month)
            .map_err(|_| AuthorityError::Decode("expiry_month is negative".to_owned()))?,
        expiry_year: u32::try_from(row.expiry_year)
            .map_err(|_| AuthorityError::Decode("expiry_year is negative".to_owned()))?,
