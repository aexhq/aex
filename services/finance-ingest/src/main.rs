//! `finance-ingest` composition root (Rust Lambda ZIP).
//!
//! Exclusive responsibility: the Stripe normalized event inbox and the authoritative
//! finance transition.
//!
...
/// Validated start-up configuration for `finance-ingest`.
///
...
pub struct Config {
    ...
    pub resource: String,
    /// Maximum accepted normalized event size in bytes.
    pub budget: u32,
}
...
pub enum RunError {
    ...
    #[error("`finance-ingest` has no implementation yet")]
    NotImplemented,
}
...
/// Environment variable naming maximum accepted normalized event size in bytes.
pub const BUDGET_VAR: &str = "AEX_MAX_EVENT_BYTES";

/// Planes this deployable may be bound to.
const PLANES: [&str; 2] = ["dev", "prd"];

impl Config {
    /// Reads and validates the configuration of `finance-ingest` from the process environment.
    ///
    /// # Errors
    ///
...
/// Runs `finance-ingest` until it stops.
///
/// # Errors
///
...
fn main() -> std::process::ExitCode {
    let config = match Config::from_env() {
        Ok(config) => config,
        Err(error) => {
            eprintln!("finance-ingest: refusing to start: {error}");
    ...
    {
        eprintln!("finance-ingest: telemetry flush left {pending} record(s) undelivered");
    }
    match outcome {
        Ok(()) => std::process::ExitCode::SUCCESS,
        Err(error) => {
            eprintln!("finance-ingest: stopped: {error}");
...
mod tests {
    ...
    fn complete() -> BTreeMap<&'static str, String> {
        BTreeMap::from([
            ...
            (RESOURCE_VAR, "aex-finance_ingest-fixture".to_owned()),
    ...
    fn accepts_a_complete_environment() {
        ...
        assert_eq!(config.resource, "aex-finance_ingest-fixture");
