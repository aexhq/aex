...
use aex_internal_contracts::usage::{ModelUsageFact, RatingMessage, RatingRequest, UsageFact};
use aex_rds_data::{
...
SELECT fact_id, intent_hash, state, rated_microusd, transaction_id FROM finance.usage_inbox \
 WHERE region = :region AND category = :category AND fact_id = :fact_id";
    ...
    /// Inserts one BYOK model observation already settled at zero dollars.
    pub const CLAIM_MODEL_FACT: &str = "\
INSERT INTO finance.usage_inbox \
  (region, category, fact_id, intent_hash, org_id, workspace_id, reservation_id, meter, basis, \
   quantity, charged_quantity, interval_start, interval_end, pricing_version, corrects_fact_id, \
   accepted_sequence, state, rated_microusd, transaction_id, provider, model, session_id, token_class) \
VALUES \
  (:region, 'model', :fact_id, :intent_hash, :org_id, :workspace_id, NULL, \
   'observability.model_tokens.v1', 'consumed', :quantity, 0, \
   TIMESTAMPTZ 'epoch' + :observed_at_ms * INTERVAL '1 millisecond', \
   TIMESTAMPTZ 'epoch' + :observed_at_ms * INTERVAL '1 millisecond', \
   NULL, NULL, :accepted_sequence, 'rated', 0, NULL, :provider, :model, :session_id, :token_class) \
ON CONFLICT (region, category, fact_id) DO NOTHING \
RETURNING fact_id, intent_hash, state, rated_microusd, transaction_id";

    /// Quarantines a conflicting observation without creating money state.
    pub const QUARANTINE_MODEL_FACT: &str = "\
UPDATE finance.usage_inbox SET state = 'quarantined', rated_microusd = NULL \
 WHERE region = :region AND category = 'model' AND fact_id = :fact_id \
   AND state = 'rated'";
...
/// A zero-dollar observability row. `rated` deliberately has no money transaction.
#[derive(Debug)]
struct ModelFactRow {
    intent_hash: [u8; 32],
    state: FactState,
        ...
        let state = match record.text(2)? {
            ...
            _ => {
                return Err(DecodeError::TypeMismatch {
                    index: 2,
                    expected: "a rated or quarantined model observation",
                });
            }
        };
        let rated_microusd = record.opt(3, Record::i64)?;
        let transaction_id = record.opt(4, Record::uuid)?;
        if (state == FactState::Rated && (rated_microusd != Some(0) || transaction_id.is_some()))
            || (state == FactState::Quarantined
                && (rated_microusd.is_some() || transaction_id.is_some()))
        {
            return Err(DecodeError::TypeMismatch {
                index: 2,
                expected: "zero-dollar model state without a money transaction",
            });
        }
        Ok(Self {
...
async fn claim_all(
    ...
    for message in messages {
        match &message.request {
            RatingMessage::Priced(request) => {
                claim_priced(transaction, organization, request, &mut outcome).await?;
            }
            RatingMessage::Model(request) => {
                claim_model(transaction, organization, request, &mut outcome).await?;
            }
        }
    }
    Ok(outcome)
...
async fn claim_priced(
    ...
    let claimed: Vec<FactRow> = transaction
        .query(Statement::with(
            ...
            vec![
                ...
                (
                    "intent_hash",
                    SqlValue::Bytes(request.intent_hash.as_bytes().to_vec()),
                ),
    ...
    if claimed.is_empty() {
        ...
        match stored {
            ...
            Some(stored) if stored.intent_hash == *request.intent_hash.as_bytes() => {
...
async fn claim_model(
    transaction: &mut Transaction<'_>,
    organization: OrganizationId,
    request: &aex_internal_contracts::usage::ModelUsageRatingRequest,
    outcome: &mut GroupOutcome,
) -> Result<(), SettleError> {
    let fact: &ModelUsageFact = &request.model_usage;
    let fact_id = fact.fact_id.to_string();
    let observed_at_ms = fact.observed_at.unix_millis();
    let class_ordinal = match fact.token_class {
        aex_internal_contracts::usage::ModelTokenClass::Input => 0,
        aex_internal_contracts::usage::ModelTokenClass::Output => 1,
        aex_internal_contracts::usage::ModelTokenClass::CacheRead => 2,
        aex_internal_contracts::usage::ModelTokenClass::CacheWrite => 3,
        aex_internal_contracts::usage::ModelTokenClass::Reasoning => 4,
    };
    let accepted_sequence = observed_at_ms
        .saturating_mul(8)
        .saturating_add(class_ordinal);
    let claimed: Vec<ModelFactRow> = transaction
        .query(Statement::with(
            sql::CLAIM_MODEL_FACT,
            vec![
                ("region", SqlValue::Text(fact.region.as_str().to_owned())),
                ("fact_id", SqlValue::Text(fact_id.clone())),
                (
                ...
                ("observed_at_ms", SqlValue::I64(observed_at_ms)),
                ("accepted_sequence", SqlValue::I64(accepted_sequence)),
                (
                    "provider",
                    SqlValue::Text(fact.provider.as_str().to_owned()),
                ),
                ("model", SqlValue::Text(fact.model.to_string())),
                ("session_id", SqlValue::Text(fact.session.to_string())),
                (
                    "token_class",
                    SqlValue::Text(fact.token_class.as_str().to_owned()),
                ),
            ],
        ))
        .await
    ...
    let stored: Option<ModelFactRow> = transaction
        .query_opt(Statement::with(
            sql::READ_FACT,
            vec![
                ("region", SqlValue::Text(fact.region.as_str().to_owned())),
                ("category", SqlValue::Text("model".to_owned())),
                ("fact_id", SqlValue::Text(fact_id.clone())),
            ],
        ))
        .await
    ...
    match stored {
        Some(stored) if stored.intent_hash == *request.intent_hash.as_bytes() => {
            outcome.duplicates += 1;
            if stored.state == FactState::Quarantined {
                outcome.quarantined.push(fact_id);
            }
        }
        Some(_) => {
        ...
        None => {
            return Err(SettleError::Unavailable(
                "model observation insert returned no row and no conflicting authority".to_owned(),
            ));
        }
    }
    Ok(())
...
mod tests {
    ...
    #[test]
    fn model_observations_settle_at_zero_without_a_rate_or_money_ledger_path() {
        assert!(sql::CLAIM_MODEL_FACT.contains("'model'"));
        assert!(sql::CLAIM_MODEL_FACT.contains("charged_quantity"));
        assert!(sql::CLAIM_MODEL_FACT.contains("'rated', 0, NULL"));
        assert!(!sql::CLAIM_MODEL_FACT.contains("pricing_context"));
        assert!(!sql::CLAIM_MODEL_FACT.contains("journal_transaction"));
        assert!(!sql::CLAIM_MODEL_FACT.contains("journal_posting"));
    }
