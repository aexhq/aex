...
//!
//! Exclusive responsibility: read-only current authorization and the 30-second
//! region/audience assertion.
//!
...
/// Validated start-up configuration for `central-authz`.
///
...
pub struct Config {
    ...
    pub region: String,
    /// KMS key that signs region/audience assertions.
    pub resource: String,
...
pub enum RunError {
    ...
    #[error("`central-authz` has no implementation yet")]
    NotImplemented,
}
...
/// Environment variable naming KMS key that signs region/audience assertions.
pub const RESOURCE_VAR: &str = "AEX_ASSERTION_SIGNING_KEY_ARN";
/// Environment variable naming assertion lifetime in seconds.
pub const BUDGET_VAR: &str = "AEX_ASSERTION_TTL_SECONDS";

/// Planes this deployable may be bound to.
const PLANES: [&str; 2] = ["dev", "prd"];

impl Config {
    /// Reads and validates the configuration of `central-authz` from the process environment.
    ///
    /// # Errors
    ///
...
mod tests {
    ...
    fn complete() -> BTreeMap<&'static str, String> {
        BTreeMap::from([
            ...
            (RESOURCE_VAR, "aex-central_authz-fixture".to_owned()),
    ...
    fn accepts_a_complete_environment() {
        ...
        assert_eq!(config.resource, "aex-central_authz-fixture");
