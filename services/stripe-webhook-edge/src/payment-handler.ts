...
function configuration(): { readonly region: string; readonly secretArn: string } {
  const apiVersion = required("AEX_STRIPE_API_VERSION");
  if (apiVersion !== STRIPE_API_VERSION) throw new Error("pinned Stripe API version mismatch");
  if (required("AEX_STRIPE_REQUEST_TIMEOUT_MS") !== "8000") {
    throw new Error("AEX_STRIPE_REQUEST_TIMEOUT_MS must be exactly 8000");
  }
  return { region: required("AEX_REGION"), secretArn: required("AEX_STRIPE_API_SECRET_ARN") };
}
...
export const paymentHandler = async (event: unknown): Promise<PaymentResult> => {
