...
//!
//! Exclusive responsibility: secret plaintext admission and immediate encryption only.
//!
...
pub struct Config {
    ...
    pub region: String,
    /// The regional-secret-custody `DynamoDB` table.
    pub resource: String,
    /// Maximum accepted plaintext size in bytes.
    pub budget: u32,
}

/// Why `regional-secret-api` refused to start.
...
pub enum RunError {
    ...
    #[error("`regional-secret-api` has no implementation yet")]
    NotImplemented,
}
...
pub const RESOURCE_VAR: &str = "AEX_SECRET_CUSTODY_TABLE";
/// Environment variable naming maximum accepted plaintext size in bytes.
pub const BUDGET_VAR: &str = "AEX_MAX_SECRET_BYTES";

/// Planes this deployable may be bound to.
const PLANES: [&str; 2] = ["dev", "prd"];

impl Config {
    /// Reads and validates the configuration of `regional-secret-api` from the process environment.
    ///
    /// # Errors
    ///
...
/// Runs `regional-secret-api` until it stops.
///
/// # Errors
///
...
mod tests {
    ...
    fn complete() -> BTreeMap<&'static str, String> {
        BTreeMap::from([
            ...
            (RESOURCE_VAR, "aex-regional_secret_api-fixture".to_owned()),
    ...
    fn accepts_a_complete_environment() {
        ...
        assert_eq!(config.resource, "aex-regional_secret_api-fixture");
