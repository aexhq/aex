...
//!
//! Exclusive responsibility: bounded OTLP admission and the durable observation receipt.
//!
...
pub struct Config {
    ...
    pub resource: String,
    /// Maximum decompressed OTLP batch size in bytes.
    pub budget: u32,
}
...
pub enum RunError {
    ...
    #[error("`regional-otlp` has no implementation yet")]
    NotImplemented,
}
...
/// Environment variable naming maximum decompressed OTLP batch size in bytes.
pub const BUDGET_VAR: &str = "AEX_MAX_DECODED_BYTES";

/// Planes this deployable may be bound to.
const PLANES: [&str; 2] = ["dev", "prd"];

impl Config {
    /// Reads and validates the configuration of `regional-otlp` from the process environment.
    ///
    /// # Errors
    ///
...
/// Runs `regional-otlp` until it stops.
///
/// # Errors
///
...
mod tests {
    ...
    fn complete() -> BTreeMap<&'static str, String> {
        BTreeMap::from([
            ...
            (RESOURCE_VAR, "aex-regional_otlp-fixture".to_owned()),
    ...
    fn accepts_a_complete_environment() {
        ...
        assert_eq!(config.resource, "aex-regional_otlp-fixture");
...
pub const ROLE: aex_observation_store_aws::composition::Role =
    aex_observation_store_aws::composition::Role::Otlp;
...
pub const REQUIRED_PROBES: &[aex_observation_store_aws::health::Probe] = &[
    ...
    aex_observation_store_aws::health::Probe::RedactionKey,
    aex_observation_store_aws::health::Probe::IngressGate,
];
