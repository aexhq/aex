# `aex-live-central-api` live tests
...
`401`), a caller-supplied `aex.*` context map gaining nothing, the two unauthenticated
device-flow routes, the merged control/auth/billing surface answering from one
listener, and readiness going `503` before the listener stops on drain.
