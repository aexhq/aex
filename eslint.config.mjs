...
// by hand. See tools/eslint-plugin-antpath/index.mjs for the rule bodies
...
//
// Adding rules: prefer adding them to the local antpath plugin so the rule
...
import antpath from "./tools/eslint-plugin-antpath/index.mjs";
...
export default tseslint.config(
  ...
  // pure noise that has nothing to do with our tests.
  {
    ignores: [
      ...
      "tools/eslint-plugin-antpath/__verify_rules.test.ts"
    ]
  },
  {
    ...
    linterOptions: {
      ...
      // care about the antpath/* rule violations in this config.
    ...
    rules: {
      ...
      "antpath/no-undefined-skip-expect": "error",
      "antpath/no-disabled-tests": "error",
      "antpath/no-focused-tests": "error",
      "antpath/no-conditional-expect": "error"
    }
  }
);
