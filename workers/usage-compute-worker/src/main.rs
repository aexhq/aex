...
//!
//! Exclusive responsibility: compute, memory, model and invocation fact projection, the
//! frontier and the central outbox.
//!
...
pub enum RunError {
    ...
    #[error("`usage-compute-worker` has no implementation yet")]
    NotImplemented,
}
...
/// Environment variable naming the usage-compute-authority `DynamoDB` table.
pub const RESOURCE_VAR: &str = "AEX_USAGE_COMPUTE_TABLE";
...
impl Config {
    /// Reads and validates the configuration of `usage-compute-worker` from the process environment.
    ///
    /// # Errors
    ///
...
mod tests {
    ...
    fn complete() -> BTreeMap<&'static str, String> {
        BTreeMap::from([
            ...
            (RESOURCE_VAR, "aex-usage_compute_worker-fixture".to_owned()),
    ...
    fn accepts_a_complete_environment() {
        ...
        assert_eq!(config.resource, "aex-usage_compute_worker-fixture");
