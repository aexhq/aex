//! `usage-receipt-dispatcher` composition root (Rust Lambda ZIP).
//!
//! Exclusive responsibility: delivery of committed finance receipts back to the regional
//! categories.
//!
...
/// Validated start-up configuration for `usage-receipt-dispatcher`.
///
...
pub struct Config {
    ...
    pub region: String,
    /// Queue carrying committed receipts to the regions.
    pub resource: String,
    /// Maximum receipts delivered per invocation.
    pub budget: u32,
}
...
/// Why `usage-receipt-dispatcher` stopped.
...
pub enum RunError {
    ...
    #[error("`usage-receipt-dispatcher` has no implementation yet")]
    NotImplemented,
}
...
/// Environment variable naming queue carrying committed receipts to the regions.
pub const RESOURCE_VAR: &str = "AEX_RECEIPT_OUTBOX_QUEUE_URL";
/// Environment variable naming maximum receipts delivered per invocation.
...
impl Config {
    /// Reads and validates the configuration of `usage-receipt-dispatcher` from the process environment.
    ///
    /// # Errors
    ///
...
/// Runs `usage-receipt-dispatcher` until it stops.
///
/// # Errors
///
...
fn main() -> std::process::ExitCode {
    let config = match Config::from_env() {
        Ok(config) => config,
        Err(error) => {
            eprintln!("usage-receipt-dispatcher: refusing to start: {error}");
    ...
    {
        eprintln!("usage-receipt-dispatcher: telemetry flush left {pending} record(s) undelivered");
    }
    match outcome {
        Ok(()) => std::process::ExitCode::SUCCESS,
        Err(error) => {
            eprintln!("usage-receipt-dispatcher: stopped: {error}");
...
mod tests {
    ...
    fn complete() -> BTreeMap<&'static str, String> {
        BTreeMap::from([
            ...
            (
                RESOURCE_VAR,
                "aex-usage_receipt_dispatcher-fixture".to_owned(),
            ),
    ...
    fn accepts_a_complete_environment() {
        ...
        assert_eq!(config.resource, "aex-usage_receipt_dispatcher-fixture");
