...
//!
//! Exclusive responsibility: exact Hands generation lifecycle, true-idle,
//! suspend/resume/terminate and usage receipts.
//!
...
pub struct Config {
    ...
    pub resource: String,
    /// Proven inactivity in milliseconds before a runtime is idle.
    pub budget: u32,
}
...
pub enum RunError {
    ...
    #[error("`runtime-control-worker` has no implementation yet")]
    NotImplemented,
}
...
pub const RESOURCE_VAR: &str = "AEX_RUNTIME_ACTIVITY_TABLE";
/// Environment variable naming proven inactivity in milliseconds before a runtime is idle.
pub const BUDGET_VAR: &str = "AEX_TRUE_IDLE_MILLIS";

/// Planes this deployable may be bound to.
const PLANES: [&str; 2] = ["dev", "prd"];

impl Config {
    /// Reads and validates the configuration of `runtime-control-worker` from the process environment.
    ///
    /// # Errors
    ///
...
mod tests {
    ...
    fn complete() -> BTreeMap<&'static str, String> {
        BTreeMap::from([
            ...
            (
                RESOURCE_VAR,
                "aex-runtime_control_worker-fixture".to_owned(),
            ),
    ...
    fn accepts_a_complete_environment() {
        ...
        assert_eq!(config.resource, "aex-runtime_control_worker-fixture");
