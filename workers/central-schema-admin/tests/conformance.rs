...
fn the_local_plan_emits_the_bound_lock_and_the_linear_head() {
    ...
    assert!(stdout.contains("\"bundleHead\":20260814000300"));
...
fn a_stale_image_is_refused_by_the_expected_bundle_head_before_anything_else() {
    ...
    assert!(stderr.contains("20260814000300"), "{stderr}");
}
...
#[test]
fn the_committed_destructive_bundle_requires_recorded_backup_evidence() {
    // Contract migrations are deliberately guarded: without release-bound
    // backup evidence the task must stop before opening a migration session.
    let output = run(&[
        "migrate",
        "--expect-head",
        "20260814000300",
    ...
    assert_eq!(
        output.status.code(),
        Some(15),
        "the destructive bundle must require backup evidence: {}",
        String::from_utf8_lossy(&output.stderr)
    );
}
