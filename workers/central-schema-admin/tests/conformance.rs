...
fn the_local_plan_emits_the_bound_lock_and_the_linear_head() {
    ...
    let stdout = String::from_utf8(output.stdout).expect("a UTF-8 receipt");
...
fn a_stale_image_is_refused_by_the_expected_bundle_head_before_anything_else() {
    ...
    assert!(stderr.contains("20260801000600"), "{stderr}");
}
...
fn the_committed_bundle_declares_nothing_destructive() {
    ...
    let output = run(&["migrate", "--expect-head", "20260801000600"]);
    assert_ne!(
        output.status.code(),
        Some(15),
