...
use central_schema_admin::runner::{applied_head, check_conservation, expect_applied_head};
...
async fn an_out_of_order_release_is_refused_before_it_applies_anything() {
    let fixture = Fixture::start().await;
    let mut connection = fixture.connect().await;
    let error = expect_applied_head(&mut connection, 20_260_801_000_600)
        .await
        .expect_err("an empty database is not at the expected head");
    assert!(error.to_string().contains("20260801000600"));
}
