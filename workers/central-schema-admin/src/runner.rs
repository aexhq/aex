...
pub struct GrantDiff {
    /// The role the difference is about.
    pub role: String,
...
impl GrantDiff {
    /// The exact statement that reconciles this one difference.
    #[must_use]
    pub fn statement(&self) -> String {
        if self.declared {
            format!(
                "GRANT {} ON {} TO {}",
                self.privilege, self.relation, self.role
            )
        } else {
            format!(
                "REVOKE {} ON {} FROM {}",
...
/// Diffs the declarative allowlist against `information_schema`.
///
/// # Errors
///
...
/// Applies a grant diff, one statement at a time.
///
/// # Errors
///
/// Returns [`RunnerError::Database`] when a statement is refused. Every
/// statement comes from [`GrantDiff::statement`], whose parts are the
/// allowlist's own identifiers and a fixed privilege word.
pub async fn apply_grants(
    connection: &mut PgConnection,
    diffs: &[GrantDiff],
) -> Result<(), RunnerError> {
    for diff in diffs {
        // Every part of the statement comes from the committed allowlist and a
        // fixed privilege word; nothing caller-supplied reaches it.
        sqlx::raw_sql(sqlx::AssertSqlSafe(diff.statement()))
...
#[cfg(test)]
mod tests {
    use super::{GrantDiff, HISTORY_SCHEMA, HISTORY_TABLE, RunnerError};
    ...
    #[test]
    fn a_grant_diff_renders_the_exact_reconciling_statement() {
        let missing = GrantDiff {
            role: "aex_finance_api".to_owned(),
            relation: "finance.account_balance".to_owned(),
        ...
        assert_eq!(
            missing.statement(),
            "GRANT SELECT ON finance.account_balance TO aex_finance_api"
        );
        let extra = GrantDiff {
            declared: false,
            actual: true,
        ...
        assert_eq!(
            extra.statement(),
            "REVOKE SELECT ON finance.account_balance FROM aex_finance_api"
        );
    }
