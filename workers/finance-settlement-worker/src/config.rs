...
mod tests {
    ...
    use super::{
        Config, ConfigError, DATABASE_ROLE_VAR, NAMESPACE, QUEUE_URL_VAR, REQUIRED_VARS,
        SERIALIZATION_RETRY_VAR,
    };
    ...
    fn a_standard_queue_is_refused_because_account_order_is_a_correctness_property() {
        ...
        assert!(
            matches!(error, ConfigError::Invalid { name, .. } if name == QUEUE_URL_VAR),
            "{error:?}"
        );
    }
    ...
    fn an_unbounded_serialization_retry_is_refused() {
        ...
        assert!(
            matches!(error, ConfigError::Invalid { name, .. } if name == SERIALIZATION_RETRY_VAR),
            "{error:?}"
        );
    }
}
