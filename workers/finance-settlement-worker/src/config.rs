...
pub const NAMESPACE: &str = "AEX_FINANCE_SETTLEMENT_";

/// The deployment plane.
pub const PLANE_VAR: &str = "AEX_FINANCE_SETTLEMENT_PLANE";
/// The bound AWS region.
pub const REGION_VAR: &str = "AEX_FINANCE_SETTLEMENT_REGION";
/// The Aurora cluster holding the `finance` schema.
pub const CLUSTER_ARN_VAR: &str = "AEX_FINANCE_SETTLEMENT_AURORA_CLUSTER_ARN";
/// The Secrets Manager secret naming this deployable's own login role.
pub const SECRET_ARN_VAR: &str = "AEX_FINANCE_SETTLEMENT_AURORA_SECRET_ARN";
/// The logical database inside the cluster.
pub const DATABASE_NAME_VAR: &str = "AEX_FINANCE_SETTLEMENT_DATABASE_NAME";
/// The `PostgreSQL` role this deployable connects as.
pub const DATABASE_ROLE_VAR: &str = "AEX_FINANCE_SETTLEMENT_DATABASE_ROLE";
/// The FIFO rating queue this worker drains.
pub const QUEUE_URL_VAR: &str = "AEX_FINANCE_SETTLEMENT_QUEUE_URL";
/// The largest number of messages settled in one organization transaction.
pub const MAX_GROUP_BATCH_VAR: &str = "AEX_FINANCE_SETTLEMENT_MAX_GROUP_BATCH";
/// How many times a serialization failure may be retried inside one invocation.
pub const SERIALIZATION_RETRY_VAR: &str = "AEX_FINANCE_SETTLEMENT_SERIALIZATION_RETRY_MAX";
/// The application deadline for one Aurora transaction.
pub const TX_DEADLINE_VAR: &str = "AEX_FINANCE_SETTLEMENT_TX_DEADLINE_MS";

/// Every variable this deployable requires, in declaration order.
pub const REQUIRED_VARS: [&str; 10] = [
    PLANE_VAR,
    REGION_VAR,
    CLUSTER_ARN_VAR,
    SECRET_ARN_VAR,
...
fn bounded<F>(lookup: &F, name: &'static str, low: u32, high: u32) -> Result<u32, FinanceSettlementWorkerConfigError>
where
    F: Fn(&str) -> Option<String>,
{
...
fn invalid(name: &'static str, error: &impl std::fmt::Display) -> FinanceSettlementWorkerConfigError {
...
mod tests {
    ...
    use super::{
        Config, FinanceSettlementWorkerConfigError, DATABASE_ROLE_VAR, NAMESPACE, QUEUE_URL_VAR, REQUIRED_VARS,
        SERIALIZATION_RETRY_VAR,
    };
    ...
    fn complete() -> BTreeMap<&'static str, String> {
        BTreeMap::from([
            ...
            (
                ...
                "aex_finance_settlement".to_owned(),
            ),
            (
                super::QUEUE_URL_VAR,
    ...
    fn read(vars: &BTreeMap<&'static str, String>) -> Result<Config, FinanceSettlementWorkerConfigError> {
    ...
    fn the_wrong_database_role_is_refused_before_a_connection_exists() {
        ...
        let error = read(&vars).expect_err("the settlement worker has its own role");
        assert!(
            matches!(error, FinanceSettlementWorkerConfigError::Invalid { name, .. } if name == DATABASE_ROLE_VAR),
            "{error:?}"
        );
    }
