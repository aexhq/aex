...
//!
//! Exclusive responsibility: spool repair, retention risk, claims, rebuild, deletion and
//! the ingress gate.
//!
...
pub struct Config {
    ...
    pub resource: String,
    /// Maximum spool claims evaluated per run.
    pub budget: u32,
}
...
pub enum RunError {
    ...
    #[error("`observation-reconciler` has no implementation yet")]
    NotImplemented,
}
...
/// Environment variable naming maximum spool claims evaluated per run.
pub const BUDGET_VAR: &str = "AEX_MAX_CLAIMS_PER_RUN";

/// Planes this deployable may be bound to.
const PLANES: [&str; 2] = ["dev", "prd"];

impl Config {
    /// Reads and validates the configuration of `observation-reconciler` from the process environment.
    ///
    /// # Errors
    ///
...
/// Runs `observation-reconciler` until it stops.
///
/// # Errors
///
...
fn main() -> std::process::ExitCode {
    let config = match Config::from_env() {
        Ok(config) => config,
        Err(error) => {
            eprintln!("observation-reconciler: refusing to start: {error}");
...
mod tests {
    ...
    fn complete() -> BTreeMap<&'static str, String> {
        BTreeMap::from([
            ...
            (
                RESOURCE_VAR,
                "aex-observation_reconciler-fixture".to_owned(),
            ),
    ...
    fn accepts_a_complete_environment() {
        ...
        assert_eq!(config.resource, "aex-observation_reconciler-fixture");
...
pub const ROLE: aex_observation_store_aws::composition::Role =
    aex_observation_store_aws::composition::Role::Reconciler;
