...
pub enum RunError {
    ...
    /// ports, and what is missing is `aex-usage-transfer-aws`'s `AuthorityStore`
    /// implementation over the real client.
    #[error("`aex-usage-transfer-aws` does not yet implement `AuthorityStore` over `DynamoDB`")]
    StoreUnimplemented,
}
...
/// # Errors
///
/// Returns [`RunError::StoreUnimplemented`] until `aex-usage-transfer-aws`
