...
fn check_charging_gate(mode: BillingMode, queue: &str) -> Result<(), UsageTransferWorkerConfigError> {
    ...
    F: Fn(&str) -> Option<String>,
{
    ...
    let value = raw.parse::<u64>().map_err(|error| UsageTransferWorkerConfigError::Invalid {
        name,
...
) -> Result<(), UsageTransferWorkerRunError> {
    telemetry.emit(
...
async fn main() -> ExitCode {
    let config = match Config::from_env() {
        ...
        Err(error) => {
            ...
            eprintln!("usage-transfer-worker: refusing to start: {error}");
    ...
    {
        eprintln!("usage-transfer-worker: telemetry flush left {pending} record(s) undelivered");
    }
    match outcome {
        Ok(()) => ExitCode::SUCCESS,
        Err(error) => {
            eprintln!("usage-transfer-worker: stopped: {error}");
            ExitCode::FAILURE
        }
    }
}
...
mod tests {
    use super::{
        ...
        BILLING_MODE_VAR, BUDGET_VAR, BillingMode, Config, UsageTransferWorkerConfigError, DispatchError, EventMode,
    ...
    fn read(vars: &BTreeMap<&'static str, String>) -> Result<Config, UsageTransferWorkerConfigError> {
    ...
    fn no_money_relevant_limit_may_be_zero_or_non_numeric() {
        for name in [
            ...
            for value in ["0", "lots", "-1"] {
                ...
                assert!(
                    matches!(read(&vars), Err(UsageTransferWorkerConfigError::Invalid { .. })),
                    "`{name}` accepted `{value}`"
                );
            }
        }
