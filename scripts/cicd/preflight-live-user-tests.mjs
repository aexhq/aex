...
export async function checkLiveUserTestsPreflight(options = {}) {
  ...
  const url = new URL("/api/files?limit=1", apiUrl);
    ...
    try {
      ...
      if (!response.ok) {
        if (attempt < attempts && isRetryablePreflightStatus(response.status)) {
          ...
          err.write(
            `live-user-tests /api/files transient HTTP ${response.status} ` +
        ...
        throw new PreflightFatalError(
          `live-user-tests /api/files preflight failed ` +
            `(status=${response.status}, requestId=${requestId}, bodyCode=${bodyCodeOf(body)}).`
        );
      }
      if (!Array.isArray(body.items)) {
        throw new PreflightFatalError(
          `live-user-tests /api/files response did not contain an items array (requestId=${requestId}).`
        );
      }
      out.write(
        `live-user-tests /api/files preflight passed ` +
      ...
      if (attempt < attempts && isRetryableFetchFailure(error)) {
        ...
        err.write(
          `live-user-tests /api/files transient fetch failure ` +
  ...
  throw new Error("live-user-tests /api/files preflight retry loop exhausted.");
}
