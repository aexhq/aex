...
## Observation engine and customer OTLP ingestion

Launch customer telemetry is generated by AEX, encoded with official
OpenTelemetry Rust types as immutable OTLP protobuf segments, and stored under
the owning session's S3 prefix for download. There is no customer OTLP write
endpoint, observation database, indexed query/search, aggregation or
quantiles, completeness-gap authority, live observation stream, or packaged
export worker. No dormant contract or deployable remains for those features.
