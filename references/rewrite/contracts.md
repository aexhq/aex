...
| C-65 | Regional usage attribution is quantity-only | The regional fold produces quantity and fact count but has no rate-book capability. Monetary output remains on central finance surfaces backed by private rate books; the public edge neither copies rates nor guesses an amount. |
| C-66 | Usage frontier names `projected` and `published`, and `serviceThrough` is optional | These are the authoritative domain stages and the actual optional coverage fact. The former `rated`/`aggregated` names described no producer. |
| C-67 | Approval status contains both `cancelled` and `expired`, and the bound call publishes all eleven revalidated fields | Withdrawal causes and deadline expiry have different semantics. Publishing config and expected revisions makes the read model match the authority record used at decision time. |
| C-68 | Durable operation failure is not an HTTP error body | `OperationFailure` carries code, retryability and optional safe detail without inventing request or operation identity from a historical step failure. |
| C-69 | Each plane's OpenAPI reachability roots include `ApiError` whenever a route declares errors | Failure responses already referenced the envelope, but the reachability walk previously considered only request and success schemas. Removing an accidental success-path reference exposed unresolved component refs; the generator now makes the error root explicit and tests every plane reference. |
