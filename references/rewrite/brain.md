...
| BR-19 | The listener port is generated from the `brain-mux` `[unit.fargate]` row into `task_shape::task_port()` — neither declared in the binary nor read from configuration | a hand-written constant is a second authority, and it drifted: the binary bound 9090 while the registry the target group and task definition are generated from declared 8080, so the task never became healthy. A defaulted-but-configurable port has the same failure with one more place to look |
...
| Eliminate the separate strongly consistent session-head read on each agent claim | Keep BR-29's activation-scoped authority and the authoritative session-head condition on every decision. First measure claim-path latency and read-capacity share under production-shape activation load. If material, stamp organization, workspace, observed deletion epoch and runtime generation onto the agent-control projection so the claim's existing `ReturnValues::AllNew` supplies them without another round trip; treat those values only as assertions, and retain the transaction's exact session-head guard against stale, deleted or cross-tenant work. Do not add a mutable per-session mux cache. |
| Prepared-effect takeover and pre-send control fencing | `mark_dispatch_started` currently conditions only the effect row's prepare-time `agentFence`; a new owner cannot take it over after claim advances the control fence, while the stale owner may still update that old row and dispatch. Define an atomic Prepared takeover/attempt transition, then update the effect and condition-check the exact current agent-control `(fence, claimOwner)` in one transaction. |
| Immediate lease release | `Committed`, `Parked` and `Abandoned` release remove `claimOwner` but retain the future expiry. A fresh owner can therefore wait the 15-second TTL plus 5-second steal grace although nobody owns the agent. Make release immediately claimable under the exact old fence/owner and prove park-to-successor and abandoned-to-failover latency. |
| Starvation-free due reconciliation | `due_scan` reads only the first bounded page, ignores `LastEvaluatedKey`, and fails the page on one malformed row. Add native continuation, per-row isolation/quarantine and a per-shard bounded cursor/sweep protocol; prove that persistent old/poison rows cannot hide younger due work. |
| Exact journal pagination and claimed-tail proof | the AWS reader derives continuation from entry count instead of DynamoDB `LastEvaluatedKey`, and activation does not prove the folded tail equals the claimed authoritative tail. Use native continuation and fail closed on gap, early EOF, hash mismatch or exact-tail mismatch before planning. |
| Large journal bodies and large model-request identity | journal write/read is inline-only despite the 32,768-byte decoder boundary, while transaction size validation can accept a larger item; the generic canonical request encoder also caps the full model request at 4 MiB. Wire immutable content references before allowing large records, and separate bounded journal canonicalization from segment-based large model-request identity. |
| Bounded large-context restore | activation cold-folds from sequence zero, the materialized snapshot and exact-revision warm cache are not wired, and wake admission reserves zero context bytes. Restore from a verified immutable snapshot plus bounded suffix, reserve measured peak resident bytes before hydration, and load-gate concurrent 1M-window shapes rather than inferring safety from the model token limit. |
| Detached-tool continuation and durable executor routing | a completed/failed detached query settles `ToolResult` then hands back without continuing or atomically creating a wake; executor routing is process-local and disappears on restart. Continue in the same activation or commit a wake with settlement, and persist/reconstruct the executor route from durable evidence. |
| Persistent root-agent run boundary | a normal assistant completion currently appends absorbing `AgentFinished`, while the public session model expects later messages on the stable root agent. Normal run terminalization must leave the root awaiting input; reserve `AgentFinished` for truly terminal child/session lifecycle. |
| Child generation propagation | the child-control `Put` omits `generationId`, while claim decoding requires it and the accepted contract says all agents in one session share the current generation. Add the canonical parent/session generation before making `OwedStep::SpawnChildren` reachable. |

### 14.1 BR-33 canonical generation and production injection

The accepted contract requires the session authority to write `generationId` as a canonical
`gen_…` identifier on every agent control row. Root control rows and Brain's claim decoder
carry that identifier; the decoder does not default, derive or convert it. The current child
fanout `Put` omits `generationId`. `OwedStep::SpawnChildren` is still unreachable, so this is
not a live path today, but child claim would fail decoding if it were connected before the
deferred propagation fix above.
