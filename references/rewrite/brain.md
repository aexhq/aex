...
| BR-39 | Cold restore has strict activation-wide entry and canonical inline-body-byte ceilings in addition to per-page bounds; mux admission reserves that payload ceiling before restore and releases it by RAII with the activation | many valid pages are still unbounded in aggregate. The code-owned [`ActivationPolicy::default`](../../crates/aex-brain-application/src/activation/mod.rs) bounds are inclusive across all service pages. They do not claim to measure DynamoDB response overhead or the decoded Rust heap. Model token limits are not memory measurements and are not used. Histories above either ceiling fail closed until a verified snapshot authority can provide a bounded suffix. |
