...
| OB-14 | The route's signal is the authority over the body's | `POST /api/observations/logs/query` carrying `signal: "metrics"` is a contradiction rather than a preference. The refusal is `invalid_query` naming both spellings. |
