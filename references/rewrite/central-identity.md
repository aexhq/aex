...
| **Settled.** `finance.account_state_v1(organization_id, status, reason, revision, changed_at)` and `finance.ensure_account(uuid)` landed in `migrations/central/20260801000700_finance_account_state.sql`, with their `SELECT` and `EXECUTE` declared in `grants.toml`. The view projects `finance.billing_account` and maps every non-`active` state — and any state added later — to `paused_top_up_required`; an organization with no row does not appear, and the reading statements `COALESCE` that to `unavailable`. The suite's view-shaped stub is deleted: it derived from `control.organization`, so every organization it knew read `active`, which is the one answer absence must never give. | finance |
