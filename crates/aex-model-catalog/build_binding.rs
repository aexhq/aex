//! Shared generator for release-bound model-catalog authority.
...
/// Generates the immutable release authority for one catalog-consuming binary.
pub fn generate(package: &str) {
    ...
    let inputs = (
        nonempty(TRUST_ROOTS_JSON_VAR),
        nonempty(TRUST_ROOTS_SHA_VAR),
        nonempty(COLLECTION_VAR),
        nonempty(COLLECTION_SHA_VAR),
    );
    let configured = [
        inputs.0.is_some(),
        inputs.1.is_some(),
    ...
    assert!(
        ...
        "{package} catalog release inputs are partial; {TRUST_ROOTS_JSON_VAR}, \
    ...
    fs::write(generated, generate_release_source(&out, inputs))
        .expect("write model catalog release binding");
}
    ...
    match inputs {
        (Some(trust_roots_json), Some(trust_roots_sha), Some(collection), Some(collection_sha)) => {
            ...
            let workspace = manifest
                ...
                .expect("catalog consumers live two directories below the workspace root");
            ...
            fs::write(out.join("model-catalog-collection.json"), bytes)
                .expect("write build-bound catalog collection");
            let entries = roots
                .keys
                .iter()
                .map(|root| {
