...
//! [`OutboxMessage`] built from an admitted `storage.byte_min.v1` fact is
//! *accepted by a real FIFO queue* — the group id, the dedupe id and both
...
//! - The `DynamoDB` half of this adapter. The `DynamoDB` Local suite —
//!   concurrent producers converging on a contiguous sequence, ambiguous-commit
//!   resolution by re-reading `ID#`, replay versus conflict discrimination — is
//!   still unwritten and still owed by the `observations-usage` stream; the
//!   expression and codec halves are covered at the unit layer. This file
//!   replaces that deferral note, not the suite it described.
...
use aex_usage_storage_dynamodb::CATEGORY;
use aex_usage_storage_dynamodb::queue::{
    BUSINESS_KEY_ATTRIBUTE, PRICING_VERSION_ATTRIBUTE, SettlementQueue,
};
use aex_usage_storage_dynamodb::stream::{ReceiptEnvelope, receipt_records};
...
const RECEIPT_QUEUE: &str = "aex-integration-usage-receipt-storage.fifo";
    ...
    Measurement::new(
        ...
        Evidence::StorageResidence {
            owner: StorageOwner {
                ...
                id: AuthorityId::parse("obj-1").expect("owner"),
                generation: 1,
            },
