...
const SIBLING_CRATES: [&str; 2] = ["aex-usage-compute-dynamodb", "aex-usage-transfer-dynamodb"];
...
fn the_category_binding_matches_the_adapter() {
    assert_eq!(
        aex_usage_storage_dynamodb::CATEGORY.id(),
        "storage",
...
fn the_table_definition_declares_exactly_the_item_types_this_category_holds() {
    ...
    let mut expected: Vec<String> = ItemType::ALL
        .into_iter()
        .filter(|item| item.allowed_in(aex_usage_storage_dynamodb::CATEGORY))
