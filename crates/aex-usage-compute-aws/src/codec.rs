...
use crate::expressions::{ComputeAuthority, FACT_BODY};
...
pub fn decode_fact(map: &HashMap<String, AttributeValue>) -> Result<UsageFact, DecodeError> {
...
pub fn decode_frontier(map: &HashMap<String, AttributeValue>) -> Result<Frontier, DecodeError> {
...
mod tests {
    ...
    use crate::expressions::{ComputeAuthority, FACT_BODY, ReceiptRow};
    ...
    fn fact_row(fact: &UsageFact) -> HashMap<String, AttributeValue> {
        attribute::item(&ComputeAuthority::new().fact_item(fact).expect("builds"))
    }
    ...
    fn a_frontier_round_trips_including_where_and_why_it_parked() {
        let advancing = Frontier::empty(region(), workspace(), crate::CATEGORY)
