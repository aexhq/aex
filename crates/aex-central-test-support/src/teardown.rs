...
pub enum CentralResource {
    /// An organization row in the control schema.
    Organization,
    /// A workspace row in the control schema.
    Workspace,
    /// An API key row and its epoch.
    ApiKey,
    /// A finance account and its balance projection.
    Account,
    /// An identity link row in the identity schema.
    IdentityLink,
    /// A balanced finance journal entry.
    JournalEntry,
}

impl CentralResource {
...
impl std::fmt::Display for CentralResource {
...
impl FixtureLedger {
    ...
    pub fn release(&mut self, kind: CentralResource, id: &str) -> bool {
...
mod tests {
    ...
    fn recording_then_releasing_leaves_the_ledger_empty() {
        ...
        assert!(ledger.release(CentralResource::Organization, "aex-test-fixture-0001"));
        assert!(ledger.is_empty());
        ledger.assert_empty();
    }
