...
//! construction and the category is [`B::CATEGORY`], so there is no code
...
use crate::expressions::{Authority, ReceiptRow, StoreError, TransactItem, WRITE_ONCE};
use crate::fault::{Idempotence, classify};
use crate::{AuthorityBinding, gsi};
...
/// The authority port implementation for the category selected by `B`.
#[derive(Debug, Clone)]
pub struct Store<B: AuthorityBinding> {
    client: Client,
    table: String,
    region: RegionId,
    clock: Arc<dyn Clock>,
    expressions: Authority<B>,
    keys: AuthorityKeys,
}

impl<B: AuthorityBinding> Store<B> {
...
#[async_trait]
impl<B: AuthorityBinding> AuthorityStore for Store<B> {
    ...
    async fn frontier(&self, workspace: &WorkspaceId) -> Result<Frontier, PortError> {
        ...
        match self.get(attribute::key(&key)).await? {
            ...
            Some(item) => codec::decode_frontier::<B>(&item).map_err(|error| decode_error(&error)),
        }
    }
    ...
    async fn fact_at(
        ...
        match self.get(attribute::key(&key)).await? {
            None => Ok(None),
            Some(item) => codec::decode_fact::<B>(&item)
                .map(Some)
            ...
            TransactWriteItem::builder()
                .update(
                    aws_sdk_dynamodb::types::Update::builder()
                        ...
                        .expression_attribute_values(":category", attribute::text(B::CATEGORY.id()))
                        .expression_attribute_values(
                            ":region",
    ...
    async fn due_outbox(
        ...
        for item in output.items.unwrap_or_default() {
            ...
            let row = codec::decode_due::<B>(&item).map_err(|error| decode_error(&error))?;
