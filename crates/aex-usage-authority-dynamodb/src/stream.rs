...
pub fn stream_records<B: AuthorityBinding>(
    ...
    for (index, record) in records.iter().enumerate() {
        decoded.push(one_record::<B>(index, record)?);
    }
    Ok(decoded)
}

/// Reads one stream record.
fn one_record<B: AuthorityBinding>(
    index: usize,
    ...
    let fact = match from_stream_json(image).and_then(|map| {
        decode_fact::<B>(&map).map_err(|error| crate::attribute::RowError::Malformed {
            attribute: "NewImage",
...
pub fn receipt_records<B: AuthorityBinding>(
    event: &serde_json::Value,
) -> Result<Vec<ReceiptRecord>, ReceiptError> {
    let records = event
        .get("Records")
        .and_then(serde_json::Value::as_array)
    ...
    for (index, record) in records.iter().enumerate() {
        ...
        let receipt = settlement_receipt::<B>(&identifier, envelope)?;
...
fn settlement_receipt<B: AuthorityBinding>(
    identifier: &str,
    envelope: ReceiptEnvelope,
) -> Result<SettlementReceipt, ReceiptError> {
    if envelope.category != B::CATEGORY.id() {
        return Err(ReceiptError::ForeignCategory {
            identifier: identifier.to_owned(),
            category: envelope.category,
            expected: B::CATEGORY.id(),
        });
    }
...
mod tests {
    use super::{
        INSERT, ReceiptEnvelope, ReceiptError, StreamError, receipt_records as receipt_records_for,
        stream_records as stream_records_for,
    };
    use crate::TestBinding;
    use crate::attribute;
    use crate::expressions::Authority;
    ...
    fn stream_records(
        ...
        stream_records_for::<TestBinding>(event)
    }

    fn receipt_records(
        event: &serde_json::Value,
    ) -> Result<Vec<aex_usage_app::worker::ReceiptRecord>, ReceiptError> {
        receipt_records_for::<TestBinding>(event)
    }
    ...
    fn fact_batch(original: &UsageFact) -> serde_json::Value {
        let row = attribute::item(&TestAuthority::new().fact_item(original).expect("builds"));
    ...
    fn an_undecodable_image_parks_at_the_position_its_key_names() {
        ...
        let original = fact(7);
        let mut row = attribute::item(&TestAuthority::new().fact_item(&original).expect("builds"));
        row.insert(
    ...
    fn a_receipt_message_decodes_including_a_reversal() {
        let records =
            receipt_records(&receipt_batch(&receipt_body(Category::Compute.id()))).expect("reads");
        assert_eq!(records.len(), 1);
        assert_eq!(records[0].identifier, "m1");
        assert_eq!(records[0].receipt.rated_microusd, -25);
        assert_eq!(records[0].receipt.category, Category::Compute);
    ...
    fn a_receipt_body_the_producer_and_consumer_disagree_about_is_refused() {
        ...
        let mut body: serde_json::Value =
            serde_json::from_str(&receipt_body(Category::Compute.id())).expect("JSON");
