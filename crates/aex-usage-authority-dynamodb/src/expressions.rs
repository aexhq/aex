//! Table expressions and the row codec shared by usage authorities.
//!
//! Every path through this module is bound to [`B::CATEGORY`]. A fact whose
...
/// The expression builder for the category selected by `B`.
#[derive(Debug, Clone, Copy)]
pub struct Authority<B: AuthorityBinding> {
    keys: AuthorityKeys,
    binding: PhantomData<fn() -> B>,
}

impl<B: AuthorityBinding> Default for Authority<B> {
    fn default() -> Self {
        Self::new()
    }
}

impl<B: AuthorityBinding> Authority<B> {
    /// Binds the builder to this crate's one category.
    #[must_use]
    pub const fn new() -> Self {
        Self {
            keys: AuthorityKeys::new(B::CATEGORY),
            binding: PhantomData,
        }
    }
    ...
    pub fn guard(self, fact: &UsageFact) -> Result<(), StoreError> {
        ...
        if category == B::CATEGORY && fact.category() == B::CATEGORY {
            return Ok(());
        }
    ...
    pub fn receipt_item(
        ...
        if receipt.category != B::CATEGORY {
        ...
        let attributes = BTreeMap::from([
            ...
            ("category".to_owned(), ItemValue::text(B::CATEGORY.id())),
            (
                "region".to_owned(),
    ...
    pub fn frontier_item(self, frontier: &Frontier) -> Result<Item, StoreError> {
        if frontier.category != B::CATEGORY {
            return Err(StoreError::CategoryEscape {
                meter: "frontier",
        ...
        Ok(Item::new(B::CATEGORY, key, ItemType::Frontier, attributes)?)
    }
    ...
    pub fn outbox_item(self, fact: &UsageFact, shard: u8) -> Result<Item, StoreError> {
        ...
        Ok(Item::new(B::CATEGORY, key, ItemType::Outbox, attributes)?)
    }
    ...
    pub fn verify_read(self, meter_id: &str) -> Result<(), StoreError> {
        if let Some(meter) = Meter::ALL.into_iter().find(|meter| meter.id() == meter_id) {
            return if meter.category() == B::CATEGORY {
                Ok(())
            } else {
                Err(StoreError::CategoryEscape {
                    meter: meter.id(),
        ...
        if B::CATEGORY == Category::Compute {
            return Ok(());
        }
        Err(StoreError::CategoryEscape {
            meter: "observability",
...
#[cfg(test)]
mod tests {
    use super::{Authority, FRONTIER_CAS, StoreError, WRITE_ONCE};
    ...
    fn siblings() -> Vec<Category> {
        Category::ALL
            .into_iter()
            .filter(|candidate| *candidate != Category::Compute)
            .collect()
    }

    #[test]
    fn the_adapter_addresses_exactly_one_authority() {
        assert_eq!(TestAuthority::new().category(), Category::Compute);
        assert_eq!(siblings().len(), 2, "there are three authorities");
    }

    #[test]
    fn a_native_fact_row_carries_its_declared_attribute_set() {
        let fact = fact_in(Category::Compute, 7);
        let item = TestAuthority::new().fact_item(&fact).expect("builds");
    ...
    fn a_sibling_authority_meter_is_refused_again_on_read() {
        ...
        for meter in Meter::ALL {
            ...
            if meter.category() == Category::Compute {
    ...
    fn a_sibling_authority_frontier_is_refused() {
        ...
        assert!(
            authority
                .frontier_item(&frontier_in(Category::Compute))
                .is_ok()
        );
    }
    ...
    fn the_admission_transaction_is_four_items_in_one_partition() {
        let fact = fact_in(Category::Compute, 1);
        let transaction = TestAuthority::new()
            .admission(&fact, &frontier_in(Category::Compute), 3)
            .expect("builds");
    ...
    fn the_claim_carries_the_intent_hash_the_replay_check_compares() {
        let fact = fact_in(Category::Compute, 4);
        let claim = TestAuthority::new().claim_item(&fact).expect("builds");
    ...
    fn the_outbox_row_carries_the_sparse_due_index_attributes() {
        let fact = fact_in(Category::Compute, 9);
        let item = TestAuthority::new().outbox_item(&fact, 5).expect("builds");
        let map = item.to_attribute_map();
        ...
        assert_eq!(map["category"], ItemValue::text(Category::Compute.id()));
    }

    #[test]
    fn a_quarantined_frontier_records_where_and_why_it_parked() {
        let parked = frontier_in(Category::Compute)
    ...
    fn a_void_records_its_lineage_and_carries_no_quantity_of_its_own() {
        let target = fact_in(Category::Compute, 1);
        let void = fact_with(
            Category::Compute,
            FactKind::Void {
    ...
    fn the_same_fact_always_produces_the_same_row() {
        ...
        let first = fact_in(Category::Compute, 3);
        let second = fact_in(Category::Compute, 3);
        let authority = TestAuthority::new();
        assert_eq!(
