//! `aex-usage-query-dynamodb` owns the read-only `usage-query-projection` adapter: bounded
...
pub use store::{AggregatePageRows, UsageProjectionReads, UsageQueryStore};
