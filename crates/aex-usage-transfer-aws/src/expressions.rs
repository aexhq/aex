//! Table expressions and the row codec for `usage-transfer-authority`.
//!
...
/// The `usage-transfer-authority` expression builder.
#[derive(Debug, Clone, Copy)]
pub struct TransferAuthority {
    keys: AuthorityKeys,
}

impl Default for TransferAuthority {
    fn default() -> Self {
        Self::new()
    }
}
...
impl TransferAuthority {
    ...
    /// # Errors
    ///
    /// As [`TransferAuthority::fact_item`].
...
#[cfg(test)]
mod tests {
    use super::{FRONTIER_CAS, StoreError, TransferAuthority, WRITE_ONCE};
    ...
    fn the_adapter_addresses_exactly_one_authority() {
        assert_eq!(TransferAuthority::new().category(), crate::CATEGORY);
    ...
    fn a_native_fact_row_carries_its_declared_attribute_set() {
        ...
        let item = TransferAuthority::new().fact_item(&fact).expect("builds");
    ...
    fn the_admission_transaction_is_four_items_in_one_partition() {
        ...
        let transaction = TransferAuthority::new()
    ...
    fn the_claim_carries_the_intent_hash_the_replay_check_compares() {
        ...
        let claim = TransferAuthority::new().claim_item(&fact).expect("builds");
    ...
    fn the_outbox_row_carries_the_sparse_due_index_attributes() {
        ...
        let item = TransferAuthority::new()
            .outbox_item(&fact, 5)
            .expect("builds");
        let map = item.to_attribute_map();
