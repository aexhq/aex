//! Table expressions and the row codec for `usage-compute-authority`.
//!
...
/// The `usage-compute-authority` expression builder.
#[derive(Debug, Clone, Copy)]
pub struct ComputeAuthority {
    keys: AuthorityKeys,
}

impl Default for ComputeAuthority {
    fn default() -> Self {
        Self::new()
    }
}

impl ComputeAuthority {
...
#[cfg(test)]
mod tests {
    use super::{ComputeAuthority, FRONTIER_CAS, StoreError, WRITE_ONCE};
    ...
    fn the_adapter_addresses_exactly_one_authority() {
        assert_eq!(ComputeAuthority::new().category(), crate::CATEGORY);
    ...
    fn a_native_fact_row_carries_its_declared_attribute_set() {
        ...
        let item = ComputeAuthority::new().fact_item(&fact).expect("builds");
    ...
    fn a_sibling_authority_fact_is_refused_on_every_write_path() {
        let authority = ComputeAuthority::new();
    ...
    fn the_admission_transaction_is_four_items_in_one_partition() {
        ...
        let transaction = ComputeAuthority::new()
    ...
    fn the_claim_carries_the_intent_hash_the_replay_check_compares() {
        ...
        let claim = ComputeAuthority::new().claim_item(&fact).expect("builds");
