//! `aex-usage-compute-dynamodb` is the `usage-compute-authority` adapter: table expressions,
//! the row codec, the admission transaction and the frontier compare-and-set for
//! `compute.millicpu_ms.v1` and `memory.byte_ms.v1` facts, plus the rating-queue producer and this
//! category's settlement receipt consumer.
//!
//! # Invariants
//!
//! - this crate can only address the compute authority; [`CATEGORY`] is the one
...
//! - the closure vector has exactly one home, and it is this table; a
//!   `CLOSURE#` row cannot be built anywhere else
//! - model tokens, provider calls and tool invocations are zero-dollar
//!   observability rows here, structurally disjoint from the priced meters
//!
//! # Not this crate's job
//!
//! - storage or transfer facts (`aex-usage-storage-dynamodb`,
//!   `aex-usage-transfer-dynamodb`); this crate depends on
...
//! - the query projection's read path (`aex-usage-query-dynamodb`)
