...
const SIBLING_CRATES: [&str; 2] = ["aex-usage-storage-dynamodb", "aex-usage-transfer-dynamodb"];

/// The two sibling authorities this crate must not name.
const SIBLING_CATEGORIES: [&str; 2] = ["storage", "transfer"];
...
fn the_category_constant_is_the_single_binding() {
    ...
    assert_eq!(declarations, 1);
    assert_eq!(
        aex_usage_compute_dynamodb::CATEGORY.id(),
        "compute",
...
fn the_table_definition_declares_exactly_the_item_types_this_category_holds() {
    ...
    let mut expected: Vec<String> = ItemType::ALL
        .into_iter()
        .filter(|item| item.allowed_in(aex_usage_compute_dynamodb::CATEGORY))
