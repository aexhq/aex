...
mod tests {
    ...
    fn two_prefixes_from_one_process_never_collide() {
        ...
        for _ in 0..512 {
            seen.push(RunPrefix::new("observation").expect("a valid label is accepted"));
        }
    ...
    fn every_prefix_is_recognisably_synthetic() {
        let prefix = RunPrefix::new("observation").expect("a valid label is accepted");
    ...
    fn resource_names_stay_inside_the_run_namespace() {
        let prefix =
            RunPrefix::deterministic("observation", "0001").expect("a valid label is accepted");
        assert_eq!(prefix.as_str(), "aex-test-observation-0001");
        assert_eq!(
            prefix
                .resource("primary")
                .expect("a valid resource name is accepted"),
            "aex-test-observation-0001-primary"
        );
    }
