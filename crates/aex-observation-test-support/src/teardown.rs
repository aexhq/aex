...
pub enum ObservationResource {
    /// An observation series claim.
    Series,
    /// An admitted observation batch and its receipt.
    Batch,
    /// An immutable observation body object.
    Body,
    /// An immutable usage fact and its correction chain.
    UsageFact,
    /// A multipart export output and its manifest.
    ExportArtifact,
}
...
impl ObservationResource {
    ...
    pub const fn as_str(self) -> &'static str {
        match self {
            ...
            Self::Batch => "batch",
            Self::Body => "body",
            Self::UsageFact => "usage_fact",
            Self::ExportArtifact => "export_artifact",
        }
    }
}

impl std::fmt::Display for ObservationResource {
...
impl FixtureLedger {
    ...
    pub fn record(&mut self, kind: ObservationResource, id: impl Into<String>) {
    ...
    pub fn release(&mut self, kind: ObservationResource, id: &str) -> bool {
    ...
    fn leak_report(&self) -> String {
        let names: Vec<String> = self
            ...
            .collect();
        format!(
            "{} aex-observation-test-support fixture ledger still holds {} resource(s): {}",
...
#[cfg(test)]
mod tests {
    use super::{FixtureLedger, ObservationResource};

    #[test]
    fn recording_then_releasing_leaves_the_ledger_empty() {
        let mut ledger = FixtureLedger::new();
        ledger.record(ObservationResource::Series, "aex-test-fixture-0001");
        assert!(!ledger.is_empty());
        assert_eq!(ledger.pending().len(), 1);
        assert!(ledger.release(ObservationResource::Series, "aex-test-fixture-0001"));
        assert!(ledger.is_empty());
        ledger.assert_empty();
    }
    ...
    fn releasing_an_unknown_resource_reports_false() {
        let mut ledger = FixtureLedger::new();
        ledger.record(ObservationResource::Series, "held");
        assert!(!ledger.release(ObservationResource::Series, "never-recorded"));
        assert_eq!(ledger.pending().len(), 1);
        ledger.drain();
    }
    ...
    fn draining_hands_every_entry_to_the_teardown_path() {
        let mut ledger = FixtureLedger::new();
        ledger.record(ObservationResource::Series, "one");
        ledger.record(ObservationResource::Batch, "two");
    ...
    fn dropping_a_non_empty_ledger_names_every_leak() {
        ...
        let outcome = std::panic::catch_unwind(|| {
            let mut ledger = FixtureLedger::new();
            ledger.record(ObservationResource::Series, "leaked-one");
            ledger.record(ObservationResource::Batch, "leaked-two");
        });
    ...
    fn a_leak_is_still_reported_when_the_test_is_already_failing() {
        ...
        let outcome = std::panic::catch_unwind(|| {
            let mut ledger = FixtureLedger::new();
            ledger.record(ObservationResource::Series, "leaked-while-failing");
