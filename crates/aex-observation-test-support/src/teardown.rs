...
impl CleanupLedger {
    ...
    fn leak_report(&self) -> String {
        let names: Vec<String> = self
            ...
            .collect();
        format!(
            "aex-observation-test-support cleanup ledger still holds {} resource(s): {}",
            names.len(),
            names.join(", ")
        )
    }
...
#[cfg(test)]
mod tests {
    use super::{CleanupLedger, ObservationResource};
    ...
    fn the_drop_check_never_masks_an_original_failure() {
        ...
        let outcome = std::panic::catch_unwind(|| {
            let mut ledger = CleanupLedger::new();
            ledger.record(ObservationResource::Series, "leaked");
