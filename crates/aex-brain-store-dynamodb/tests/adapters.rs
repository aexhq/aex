...
use aex_brain_store_dynamodb::journal::{condition_for, dispatch_order};
use aex_brain_store_dynamodb::{BrainStore, BrainTables, DueScan, SqsWakeQueue};
...
/// A store whose two `GetItem` calls are answered with the given HTTP statuses and bodies.
///
/// The probe's contract is about *which* table refused, so the two responses have to be
/// separately controllable rather than uniformly successful.
fn answering(responses: Vec<(u16, serde_json::Value)>) -> (BrainStore, StaticReplayClient) {
    let events = responses
        .into_iter()
        .map(|(status, response)| {
            ReplayEvent::new(
...
/// A wake queue whose `SQS` transport is captured rather than its `DynamoDB` backstop.
fn capturing_wake_queue_transport() -> (SqsWakeQueue, CaptureRequestReceiver) {
    let (dynamo_http, _unused) = capture_request(None);
    let dynamo = aws_sdk_dynamodb::Config::builder()
        .behavior_version_latest()
        .region(aws_sdk_dynamodb::config::Region::new("eu-west-1"))
        .credentials_provider(aws_sdk_dynamodb::config::Credentials::for_tests())
        .http_client(dynamo_http)
        .build();
    let (sqs_http, receiver) = capture_request(None);
    let sqs = aws_sdk_sqs::Config::builder()
        .behavior_version_latest()
        .region(aws_sdk_sqs::config::Region::new("eu-west-1"))
        .credentials_provider(aws_sdk_sqs::config::Credentials::for_tests())
...
async fn a_journal_page_carries_the_complete_native_continuation() {
    let (store, receiver) = capturing();
    let partition = aex_brain_store_dynamodb::keys::agent_partition(&key()).expect("a valid key");
    let cursor = JournalCursor::new(
        [
            (aex_session_dynamodb::attr::PK, partition.clone()),
            (
                aex_session_dynamodb::attr::SK,
                aex_brain_store_dynamodb::keys::journal_sort_key(JournalSeq(4)),
            ),
        ],
        JournalSeq(4),
        JournalSeq(5),
...
fn control_item() -> aex_session_dynamodb::attr::Item {
    let control = aex_brain_store_dynamodb::keys::control(&key()).expect("a valid key");
...
fn effect_item(seed: u8, state: &str) -> aex_session_dynamodb::attr::Item {
    ...
    let effect = aex_brain_store_dynamodb::keys::effect(&key(), id).expect("a valid key");
...
fn entry(seq: u64, record: &JournalRecord) -> aex_session_dynamodb::attr::Item {
    ...
    ItemBuilder::new(aex_session_dynamodb::codec::JOURNAL_ENTRY)
        .set(
            ...
            s(aex_brain_store_dynamodb::keys::agent_partition(&key()).expect("a valid key")),
        )
        .set(
            aex_session_dynamodb::attr::SK,
...
fn partition() -> String {
    aex_brain_store_dynamodb::keys::agent_partition(&key()).expect("a valid key")
}
...
fn a_contiguous_page_decodes_in_order() {
    ...
    let page = aex_brain_store_dynamodb::journal::decode_page(
        &items,
        None,
        &partition(),
        JournalSeq(4),
...
fn a_page_that_observes_a_gap_returns_no_entries_at_all() {
    ...
    let error = aex_brain_store_dynamodb::journal::decode_page(
        &items,
        None,
        &partition(),
        JournalSeq(4),
...
fn every_native_continuation_component_is_required_and_validated() {
    ...
    let missing = aex_brain_store_dynamodb::journal::decode_page(
        &items,
        Some(&missing_sk),
        &partition(),
        JournalSeq(4),
    ...
    let wrong = aex_brain_store_dynamodb::journal::decode_page(
        &items,
    ...
    let extra = aex_brain_store_dynamodb::journal::decode_page(
        &items,
        Some(&extra),
        &partition(),
        JournalSeq(4),
...
fn every_participant_names_the_failure_it_means() {
    ...
    assert!(matches!(
        condition_for(aex_brain_store_dynamodb::plan::participant::SESSION_BUDGET),
        ConditionFailure::BudgetExhausted
    ));
    assert!(matches!(
        condition_for(aex_brain_store_dynamodb::plan::participant::CHILD_INDEX),
...
/// A readiness probe that only constructed a client would report ready against a deleted
/// table, so the probe is the read the serving path performs: strongly consistent, on both
/// tables, against a reserved identity whose absence is the success.
#[tokio::test]
async fn the_store_probe_reads_a_reserved_absent_key_on_each_table_it_addresses() {
    let (store, replay) = answering(vec![
        (200, serde_json::json!({})),
        (200, serde_json::json!({})),
    ]);
    store
        .probe()
        .await
        .expect("a response carrying no item is what proves reachability");

    let requests = captured_requests(&replay);
    assert_eq!(requests.len(), 2, "each table fails independently");
    assert_eq!(requests[0]["TableName"], "dev-eu-west-1-session-authority");
    assert_eq!(requests[0]["ConsistentRead"], true);
    assert_eq!(requests[0]["Key"]["sk"]["S"], "CONTROL");
    assert_eq!(
        requests[0]["Key"]["pk"]["S"],
        serde_json::Value::String(
            aex_brain_store_dynamodb::keys::agent_partition(
                &aex_brain_store_dynamodb::keys::health_probe_agent()
            )
            .expect("the reserved identity renders")
        )
    );
    assert_ne!(
        requests[0]["Key"]["pk"]["S"],
...
/// Each table is separately load-bearing, so each refusal has to fail readiness by itself
/// and say which authority answered.
#[tokio::test]
async fn either_table_refusing_fails_the_store_probe_by_name() {
    let denied = serde_json::json!({ "__type": "com.amazon.coral.service#AccessDeniedException" });
    let (store, _replay) = answering(vec![(400, denied.clone())]);
    let error = store
        .probe()
        .await
        .expect_err("a refused session-authority read is not reachability");
    assert!(
        matches!(&error, StoreError::Transport { reason, .. } if reason.contains("probe_session_authority")),
        "{error:?}"
    );

    let (store, replay) = answering(vec![(200, serde_json::json!({})), (400, denied)]);
    let error = store
        .probe()
        .await
        .expect_err("a refused regional-work read is not reachability");
    assert!(
        matches!(&error, StoreError::Transport { reason, .. } if reason.contains("probe_regional_work")),
        "{error:?}"
    );
    assert_eq!(
        captured_requests(&replay).len(),
        2,
        "the session-authority read succeeded before regional-work refused"
    );
}

/// `ReceiveMessage` would take a delivery out of the queue, start its visibility timeout and
/// race the pump for the work the probe exists to report on.
#[tokio::test]
async fn the_wake_queue_probe_asks_for_an_attribute_rather_than_receiving() {
    let (queue, receiver) = capturing_wake_queue_transport();
    let _ = queue.probe().await;
    let request = receiver.expect_request();
    let body = String::from_utf8_lossy(request.body().bytes().unwrap_or_default()).into_owned();
    assert!(
        request
            .headers()
            .get("x-amz-target")
            .is_some_and(|target| target.ends_with("GetQueueAttributes")),
        "{:?}",
        request.headers().get("x-amz-target")
    );
    assert!(body.contains(queue.queue_url()), "{body}");
    assert!(body.contains("QueueArn"), "{body}");
    assert!(!body.contains("WaitTimeSeconds"), "{body}");
    assert!(
        aex_brain_store_dynamodb::WAKE_QUEUE_IAM_ACTIONS.contains(&"sqs:GetQueueAttributes"),
        "the operation readiness depends on has to be part of the declared queue grant"
    );
}
