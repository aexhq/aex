...
fn builtin_catalog_preserves_the_clean_cut_and_runtime_semantics() {
    ...
    assert_eq!(
        ...
        "the six Tool Mux targets plus three Brain-native tools are the complete MVP catalog"
    );
...
fn storage_persist_is_the_provider_safe_spelling_of_the_product_tool() {
    ...
    assert_eq!(tool.descriptor.route, ExecutorRoute::PlatformStorage);
...
fn advertisement_is_exactly_the_ready_session_local_tools() {
    ...
    assert_eq!(advertised.entries.len(), 9);
    for name in [
        "bash",
        "create_subagent",
        "edit_file",
