...
async fn the_store_probe_reads_a_reserved_absent_key_on_each_table_it_addresses() {
    ...
    assert_eq!(
        ...
        serde_json::Value::String(
            aex_brain_store_aws::keys::agent_partition(
                &aex_brain_store_aws::keys::health_probe_agent()
            )
    ...
    assert_ne!(
        requests[0]["Key"]["pk"]["S"],
        serde_json::Value::String(aex_brain_store_aws::keys::agent_partition(&key()).expect("v7")),
    ...
    assert_eq!(
        requests[1]["Key"]["pk"]["S"],
        format!("WORK#{}", aex_brain_store_aws::keys::HEALTH_PROBE_WORK_ID)
    );
...
async fn the_wake_queue_probe_asks_for_an_attribute_rather_than_receiving() {
    ...
    assert!(
        aex_brain_store_aws::WAKE_QUEUE_IAM_ACTIONS.contains(&"sqs:GetQueueAttributes"),
