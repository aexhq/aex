...
fn abi_examples_validate_and_round_trip() {
    for (name, type_name, value) in examples("abi") {
        validate(aex_contracts::ABI_SCHEMA_JSON, &name, &type_name, &value);
...
fn every_session_event_type_has_an_example() {
    let schema: Value = serde_json::from_str(aex_contracts::SESSION_SCHEMA_JSON).unwrap();
