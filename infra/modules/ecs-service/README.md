...
second task would take turns it has no state for. The pin is enforced by
`desired_count`, the capacity bounds and their validations, and it comes off
when the multi-task session-affinity decision lands, not before.
